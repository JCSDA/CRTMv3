...
!
!       Modified by:    Quanhua (Mark) Liu, 18-Nov-2020
...
CONTAINS
  ...
  FUNCTION CRTM_Tangent_Linear( &
    ...
    !** openMP over channels starts here
    n_channel_threads = 1
  ...
  CONTAINS
    ...
    FUNCTION profile_solution (m, Opt, AncillaryInput) RESULT( Error_Status )
      ...
      TYPE(CRTM_AtmOptics_type)  :: AtmOptics_Clear(n_channel_threads),AtmOptics_Clear_TL(n_channel_threads)
      TYPE(CRTM_SfcOptics_type)  :: SfcOptics_Clear(n_channel_threads),SfcOptics_Clear_TL(n_channel_threads)
      TYPE(CRTM_RTSolution_type) :: RTSolution_Clear(n_channel_threads),RTSolution_Clear_TL(n_channel_threads)
      ...
      TYPE(CRTM_Predictor_type)    :: Predictor(n_channel_threads),Predictor_TL(n_channel_threads)
      TYPE(CRTM_AtmOptics_type)    :: AtmOptics(n_channel_threads),AtmOptics_TL(n_channel_threads)
      TYPE(CRTM_SfcOptics_type)    :: SfcOptics(n_channel_threads),SfcOptics_TL(n_channel_threads)
      ...
      DO nt = 1, n_channel_threads
        ...
        IF ( (.NOT. CRTM_SfcOptics_Associated(SfcOptics(nt)  )) .OR. &
          (.NOT. CRTM_SfcOptics_Associated(SfcOptics_TL(nt))) ) THEN
          Error_Status = FAILURE
        ...
        SfcOptics_TL(nt)%n_Stokes = RTV(nt)%n_Stokes
      END DO
      ...
      DO nt = 1, n_channel_threads
        ...
        CALL CRTM_AtmOptics_Create( AtmOptics_TL(nt)    , &
                                    Atm%n_Layers        , &
                                    MAX_N_LEGENDRE_TERMS, &
                                    CloudC%N_PHASE_ELEMENTS)
      ...
      DO nt = 1, n_channel_threads
        ...
        CALL CRTM_Compute_SurfaceT_TL( Surface(m), Surface_TL(m), SfcOptics_TL(nt) )
      END DO
      ...
      Sensor_Loop: DO n = 1, n_Sensors
        ...
        Channel_Loop: DO nt = 1, n_channel_threads
          ...
          IF ( (.NOT. CRTM_Predictor_Associated(Predictor(nt))) .OR. &
            (.NOT. CRTM_Predictor_Associated(Predictor_TL(nt))) ) THEN
            Error_Status=FAILURE
          ...
          IF ( ( Atm%n_Clouds   > 0 .OR. &
            ...
            IF ( .NOT. RTV_Associated(RTV(nt)) ) THEN
              Error_Status=FAILURE
              WRITE( Message,'("Error allocating RTV structure for profile #",i0, " and ",a," sensor.")' ) &
                m, TRIM(SC(SensorIndex)%Sensor_Id)
              CALL Display_Message( ROUTINE_NAME, Message, Error_Status )
            END IF
          END IF
        END DO Channel_Loop
        ...
        Thread_Loop: DO nt = 1, n_channel_threads
          ...
          ! CHANNEL LOOP
          ! -------------
          Channel_Loop2: DO l = start_ch, end_ch
            
            ! Channel setup
            ! ...Skip channel if requested
            IF ( .NOT. ChannelInfo(n)%Process_Channel(l) ) CYCLE Channel_Loop2
            ! ...Shorter name
            ...
            RTSolution_TL(ln,m)%Sensor_Id        = ChannelInfo(n)%Sensor_Id
            RTSolution_TL(ln,m)%WMO_Satellite_Id = ChannelInfo(n)%WMO_Satellite_Id
            RTSolution_TL(ln,m)%WMO_Sensor_Id    = ChannelInfo(n)%WMO_Sensor_Id
            RTSolution_TL(ln,m)%Sensor_Channel   = ChannelInfo(n)%Sensor_Channel(l)
            
            
            ! Initialisations
            CALL CRTM_AtmOptics_Zero( AtmOptics(nt) )
            CALL CRTM_AtmOptics_Zero( AtmOptics_TL(nt) )
            ...
            CALL CRTM_Compute_AtmAbsorption_TL( SensorIndex     , &  ! Input
                                                ChannelIndex    , &  ! Input
                                                Predictor(nt)   , &  ! Input
                                                Predictor_TL(nt), &  ! Input
                                                AtmOptics_TL(nt), &  ! Output
            ...
            IF((SpcCoeff_IsVisibleSensor(SC(SensorIndex)).OR.SpcCoeff_IsUltravioletSensor(SC(SensorIndex))) &
              ...
              IF ( Status_FWD /= SUCCESS .OR. Status_TL /= SUCCESS) THEN
                ...
                WRITE( Message,'("Error computing MoleculeScatter for ",a,&
                  ...
                  ChannelInfo(n)%Sensor_Channel(l), m
                CALL Display_Message( ROUTINE_NAME, Message, Error_Status )
                CYCLE  !RETURN
              END IF
            ELSE
            ...
            IF ( CRTM_Atmosphere_IsFractional(cloud_coverage_flag) ) THEN
              ...
              Status_TL  = CRTM_AtmOptics_NoScatterCopy_TL( AtmOptics(nt), AtmOptics_TL(nt), AtmOptics_Clear_TL(nt) )
              IF ( Status_FWD /= SUCCESS .OR. Status_TL /= SUCCESS ) THEN
                Error_Status = FAILURE
            ...
            CALL CRTM_Compute_Transmittance_TL(AtmOptics(nt),AtmOptics_TL(nt),transmittance_TL)
            SfcOptics_TL(nt)%Transmittance = transmittance_TL
            ...
            IF ( CRTM_Atmosphere_IsFractional(cloud_coverage_flag) ) THEN
              ...
              CALL CRTM_Compute_Transmittance_TL(AtmOptics_Clear(nt),AtmOptics_Clear_TL(nt),transmittance_clear_TL)
              SfcOptics_Clear_TL(nt)%Transmittance = transmittance_clear_TL
            END IF
            ...
            RTSolution_TL(ln,m)%Stokes(:) = ZERO
            RTSolution_TL(ln,m)%Brightness_Temperature = ZERO
            ...
            Azimuth_Fourier_Loop: DO mth_Azi = 0, RTV(nt)%n_Azi
              ...
              Error_Status = CRTM_Compute_RTSolution_TL( &
                                    ...
                                    AtmOptics_TL(nt)   , &  ! TL  Input
                                    SfcOptics_TL(nt)   , &  ! TL  Input
                                    GeometryInfo       , &  ! Input
                                    SensorIndex        , &  ! Input
                                    ChannelIndex       , &  ! Input
              ...
              IF (CRTM_Atmosphere_IsFractional(cloud_coverage_flag).AND.RTV(nt)%mth_Azi==0 ) THEN
                ...
                Error_Status = CRTM_Compute_RTSolution_TL(     &
                                      ...
                                      Surface_TL(m)          , &  ! TL  Input
                                      AtmOptics_Clear_TL(nt) , &  ! TL  Input
                                      SfcOptics_Clear_TL(nt) , &  ! TL  Input
                                      GeometryInfo           , &  ! Input
                                      SensorIndex            , &  ! Input
                                      ChannelIndex           , &  ! Input
                                      RTSolution_Clear_TL(nt), &  ! TL  Output
            ...
            IF ( CRTM_Atmosphere_IsFractional(cloud_coverage_flag) ) THEN
              ...
              DO ks = 1, RTV(nt)%n_Stokes
                ...
                RTSolution_TL(ln,m)%Stokes(ks) = &
                  ((r_cloudy - RTSolution_Clear(nt)%Stokes(ks)) * CloudCover_TL%Total_Cloud_Cover) + &
                  ((ONE - CloudCover%Total_Cloud_Cover)   *  RTSolution_Clear_TL(nt)%Stokes(ks)  ) + &
                  (CloudCover%Total_Cloud_Cover           *  RTSolution_TL(ln,m)%Stokes(ks)      )
    ...
    SUBROUTINE Post_Process_RTSolution(Opt, rts, rts_TL, NLTE_Predictor, NLTE_Predictor_TL, &
               ChannelIndex, SensorIndex, compute_antenna_correction, GeometryInfo)
      TYPE(CRTM_Options_type), INTENT(IN) :: Opt
