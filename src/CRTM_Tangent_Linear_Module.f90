...
!     Added openMP      Quanhua (Mark) Liu, 18-11-2020
...
!$OMP PARALLEL DO NUM_THREADS(n_channel_threads)
      DO nt = 1, n_channel_threads
        ...
        SfcOptics(nt)%Use_PARMIO_Model = Opt%Use_PARMIO_Model
        SfcOptics_TL(nt)%Use_PARMIO_Model = Opt%Use_PARMIO_Model
      END DO
!$OMP END PARALLEL DO
...
!$OMP PARALLEL DO NUM_THREADS(n_channel_threads) PRIVATE(Message)
     Para_Loop1: DO nt = 1, n_channel_threads
        ...
        END IF
          ...
          SfcOptics_Clear(nt)%Use_PARMIO_Model = Opt%Use_PARMIO_Model
          SfcOptics_Clear_TL(nt)%Use_PARMIO_Model = Opt%Use_PARMIO_Model
...
!        chunk_ch = n_sensor_channels / n_channel_threads
        ...
        ! (each thread touches only its own slice) rather than PRIVATE. Error
...
!  non scattering case, this condition may be changed for future surface reflectance
      IF( .not.RTSolution(ln,m)%Scattering_FLAG .or. .not.AtmOptics(nt)%Include_Scattering ) RTV(nt)%n_Azi = 0
          ...
          Azimuth_Fourier_Loop: DO mth_Azi = 0, RTV(nt)%n_Azi
            ...
            Err_Thread = CRTM_Compute_RTSolution_TL( &
                             Atm                , &  ! FWD Input
                             Surface(m)         , &  ! FWD Input
                             AtmOptics(nt)          , &  ! FWD Input
