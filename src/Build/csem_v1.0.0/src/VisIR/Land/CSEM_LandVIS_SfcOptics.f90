...
!:sdoc+:
!
! CSEM_LandVIS_SfcOptics
!
! Module to compute the surface optical properties of Land surfaces at VISIBLE 
...
! functions as those in the original CRTM surface module "CRTM_LandVIS_SfcOptics", 
...
MODULE CSEM_LandVIS_SfcOptics
  ...
  ! Data types
  ! Science routines
  PUBLIC :: CSEM_Compute_LandVIS_SfcOptics
  PUBLIC :: CSEM_Compute_LandVIS_SfcOptics_TL
  PUBLIC :: CSEM_Compute_LandVIS_SfcOptics_AD
  PUBLIC :: iVar_type
...
!       frequencies over a Land surface.
!
!       This function encapsulates all available  CSEM LandVIS emissivity models,and
!       provides a genereic interface for the upper-level user application.
!
! CALLING SEQUENCE:
!       IO_Status = CSEM_Compute_LandVIS_SfcOptics( 
!                                        Surface                  ,&  ! Input
!                                        SfcOptics                ,&  ! Output
...
!                        be used by emprirical and semi-empirical models
!                        UNITS:      N/A
!                        TYPE:       INTEGER
!                        DIMENSION:  Scalar
!                        ATTRIBUTES: INTENT(IN)
...
!-----------------------------------------------------------------------------------------------
  ...
  FUNCTION CSEM_Compute_LandVIS_SfcOptics( &
      Surface,                             &  ! Input
      SfcOptics,                           &  ! Output
      Options )                            &  ! Input 
    RESULT (IO_Status)
    ...
    ! Local parameters
    CHARACTER(*), PARAMETER :: ROUTINE_NAME = 'CSEM_Compute_LandVIS_SfcOptics'
    ! Local variables
    ...
    SELECT CASE (TRIM(MODEL%NAME))
      ...
      CASE DEFAULT
        IF(Surface%Land_Cover_Type > 20) THEN
           Message = 'NPOESS_LUT  Land Cover_Type 1-20'
  ...
  END FUNCTION CSEM_Compute_LandVIS_SfcOptics
...
!       reflectivity at visible frequencies over a Land surface.
!
!       This function is a wrapper for third party code.
!
!
! CALLING SEQUENCE:
!       IO_Status = CSEM_Compute_LandVIS_SfcOptics_TL( SfcOptics_TL )
!
! OUTPUTS:
...
!----------------------------------------------------------------------------------
  ...
  FUNCTION CSEM_Compute_LandVIS_SfcOptics_TL(  &
    ...
    ! Local parameters
    CHARACTER(*), PARAMETER :: ROUTINE_NAME = 'CSEM_Compute_LandVIS_SfcOptics_TL'
  ...
  END FUNCTION CSEM_Compute_LandVIS_SfcOptics_TL
...
!
! CALLING SEQUENCE:
!       IO_Status = CSEM_Compute_LandVIS_SfcOptics_AD( SfcOptics_AD )
!
! INPUTS:
...
!----------------------------------------------------------------------------------
  ...
  FUNCTION CSEM_Compute_LandVIS_SfcOptics_AD(  &
       Surface_AD)                             &  ! AD  Input
    RESULT( IO_Status )
    ...
    INTEGER :: IO_Status
    ! Local parameters
    CHARACTER(*), PARAMETER :: ROUTINE_NAME = 'CSEM_Compute_LandVIS_SfcOptics_AD'
  ...
  END FUNCTION CSEM_Compute_LandVIS_SfcOptics_AD


END MODULE CSEM_LandVIS_SfcOptics
