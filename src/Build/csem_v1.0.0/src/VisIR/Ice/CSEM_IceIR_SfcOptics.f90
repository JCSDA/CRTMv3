...
!:sdoc+:
!
! CSEM_IceIR_SfcOptics
!
! Module to compute the surface optical properties of ICE surfaces at infrared 
...
! functions as those in the original CRTM surface module "CRTM_IceIR_SfcOptics", 
...
MODULE CSEM_IceIR_SfcOptics
  ...
  ! Data types
  ! Science routines
  PUBLIC :: CSEM_Compute_IceIR_SfcOptics
  PUBLIC :: CSEM_Compute_IceIR_SfcOptics_TL
  PUBLIC :: CSEM_Compute_IceIR_SfcOptics_AD
  PUBLIC :: iVar_type
...
!       frequencies over a Ice surface.
!
!       This function encapsulates all available  CSEM IceIR emissivity models,and
!       provides a genereic interface for the upper-level user application.
!
! CALLING SEQUENCE:
!       IO_Status = CSEM_Compute_IceIR_SfcOptics( 
!                                        Surface                  ,&  ! Input
!                                        SfcOptics                ,&  ! Output
...
!-----------------------------------------------------------------------------------------------
  ...
  FUNCTION CSEM_Compute_IceIR_SfcOptics (  &
      Surface,                             &  ! Input
      SfcOptics,                           &  ! Output
    ...
    ! Local parameters
    CHARACTER(*), PARAMETER :: ROUTINE_NAME = 'CSEM_Compute_IceIR_SfcOptics'
    ...
    SELECT CASE (TRIM(MODEL%NAME))
      ...
      IF(Surface%Ice_Type /=1 ) THEN
         Message = 'NPOESS_LUT Ice Type should be 1'
  ...
  END FUNCTION CSEM_Compute_IceIR_SfcOptics
...
!       reflectivity at infrared frequencies over a Ice surface.
!
!       This function is a wrapper for third party code.
!
!
! CALLING SEQUENCE:
!       IO_Status = CSEM_Compute_IceIR_SfcOptics_TL( SfcOptics_TL )
!
! OUTPUTS:
...
!----------------------------------------------------------------------------------
  ...
  FUNCTION CSEM_Compute_IceIR_SfcOptics_TL ( &
    ...
    ! Local parameters
    CHARACTER(*), PARAMETER :: ROUTINE_NAME = 'CSEM_Compute_IceIR_SfcOptics_TL'
  ...
  END FUNCTION CSEM_Compute_IceIR_SfcOptics_TL
...
!
! CALLING SEQUENCE:
!       IO_Status = CSEM_Compute_IceIR_SfcOptics_AD( SfcOptics_AD )
!
! INPUTS:
...
!----------------------------------------------------------------------------------
  ...
  FUNCTION CSEM_Compute_IceIR_SfcOptics_AD ( &
       Surface_AD)                           &  ! AD  Input
    RESULT( IO_Status )
    ...
    INTEGER :: IO_Status
    ! Local parameters
    CHARACTER(*), PARAMETER :: ROUTINE_NAME = 'CSEM_Compute_IceIR_SfcOptics_AD'
  ...
  END FUNCTION CSEM_Compute_IceIR_SfcOptics_AD

END MODULE CSEM_IceIR_SfcOptics
