...
!:sdoc+:
!
! CSEM_IceVIS_SfcOptics
!
! Module to compute the surface optical properties of ICE surfaces at VISIBLE 
...
! functions as those in the original CRTM surface module "CRTM_IceVIS_SfcOptics", 
...
MODULE CSEM_IceVIS_SfcOptics
  ...
  ! Data types
  ! Science routines
  PUBLIC :: CSEM_Compute_IceVIS_SfcOptics
  PUBLIC :: CSEM_Compute_IceVIS_SfcOptics_TL
  PUBLIC :: CSEM_Compute_IceVIS_SfcOptics_AD
  PUBLIC :: iVar_type
...
!
!       This function is a wrapper for third party codes.
!
! PURPOSE:
!       Function to compute the surface emissivity  at visible
!       frequencies over a Ice surface.
!
!       This function encapsulates all available  CSEM IceVIS emissivity models,and
!       provides a genereic interface for the upper-level users' applications.
!
! CALLING SEQUENCE:
!       IO_Status = CSEM_Compute_IceVIS_SfcOptics( 
!                                        Surface                  ,&  ! Input
!                                        SfcOptics                ,&  ! Output
...
!
!       Options:         CSEM extension inputs structure containing additional inputs,
...
!-----------------------------------------------------------------------------------------------
  ...
  FUNCTION CSEM_Compute_IceVIS_SfcOptics(  &
      Surface,                             &  ! Input
      SfcOptics,                           &  ! Output
      Options)                             &  ! Input
    RESULT (IO_Status)
    ...
    ! Local parameters
    CHARACTER(*), PARAMETER :: ROUTINE_NAME = 'CSEM_Compute_IceVIS_SfcOptics'
    ! Local variables
    ...
    SELECT CASE (TRIM(MODEL%NAME))
      ...
      CASE DEFAULT
        IF(Surface%Ice_Type /= 1 ) THEN
           Message = 'NPOESS_LUT  Ice Type 1'
  ...
  END FUNCTION CSEM_Compute_IceVIS_SfcOptics
...
!       reflectivity at visible frequencies over a Ice surface.
!
!       This function is a wrapper for third party code.
!
!
! CALLING SEQUENCE:
!       IO_Status = CSEM_Compute_IceVIS_SfcOptics_TL( SfcOptics_TL )
!
! OUTPUTS:
...
!----------------------------------------------------------------------------------
  ...
  FUNCTION CSEM_Compute_IceVIS_SfcOptics_TL(  &
    ...
    ! Local parameters
    CHARACTER(*), PARAMETER :: ROUTINE_NAME = 'CSEM_Compute_IceVIS_SfcOptics_TL'
  ...
  END FUNCTION CSEM_Compute_IceVIS_SfcOptics_TL
...
!
! CALLING SEQUENCE:
!       IO_Status = CSEM_Compute_IceVIS_SfcOptics_AD( SfcOptics_AD )
!
! INPUTS:
...
!----------------------------------------------------------------------------------
  ...
  FUNCTION CSEM_Compute_IceVIS_SfcOptics_AD(  &
        Surface_AD)                           &  ! AD  Input
    RESULT( IO_Status )
    ...
    INTEGER :: IO_Status
    ! Local parameters
    CHARACTER(*), PARAMETER :: ROUTINE_NAME = 'CSEM_Compute_IceVIS_SfcOptics_AD'
    ...
    Surface_AD%Ice_Temperature  = Surface_AD%Ice_Temperature + ZERO
    Surface_AD%Ice_Density      = Surface_AD%Ice_Density     + ZERO
    Surface_AD%Ice_Thickness    = Surface_AD%Ice_Thickness   + ZERO
    Surface_AD%Ice_Roughness    = Surface_AD%Ice_Roughness   + ZERO
    Surface_AD%Salinity         = Surface_AD%Salinity        + ZERO

  END FUNCTION CSEM_Compute_IceVIS_SfcOptics_AD

END MODULE CSEM_IceVIS_SfcOptics
