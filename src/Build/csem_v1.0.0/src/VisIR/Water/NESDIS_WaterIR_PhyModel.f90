...
!       Modified by:    Ming Chen, 25-Mar-2015
!                       ming.chen@noaa.gov
!

MODULE NESDIS_WaterIR_PhyModel
  ...
  USE NESDIS_WaterIR_Emiss_Module,   ONLY: IRSSEM_type=>Einterp_type, &
                                      IRSSEM_Setup,                   &
                                      IRSSEM_CleanUp,                 &
                                      IRSSEM_Initialized,             &
  ...
  PUBLIC :: iVar_type
  ! Science routines
  PUBLIC :: NESDIS_IRSSEM_BRDF
  PUBLIC :: NESDIS_IRSSEM_BRDF_TL
  PUBLIC :: NESDIS_IRSSEM_BRDF_AD
  PUBLIC :: NESDIS_IRSSEM_Setup
  PUBLIC :: NESDIS_IRSSEM_Close
  PUBLIC :: NESDIS_IRSSEM_Initialized
  
  INTERFACE NESDIS_IRSSEM_BRDF
    MODULE PROCEDURE Compute_WaterIR_Emiss
    MODULE PROCEDURE Compute_WaterIR_Emiss_BRDF
  END INTERFACE NESDIS_IRSSEM_BRDF
...
!
! CALLING SEQUENCE:
!       Error_Status = Compute_WaterIR_Emiss( &
!                        Wavenumber           , &  ! Input
!                        Wind_Speed           , &  ! Input
!                        Angle                , &  ! Input
!                        Emissivity           , &  ! Output
...
!:sdoc-:
!----------------------------------------------------------------------------------
  FUNCTION Compute_WaterIR_Emiss( &
    Wavenumber                    , &  ! Input
    Wind_Speed                    , &  ! Input
    Angle                         , &  ! Input
    Emissivity                    , &  ! Output
  ...
  RESULT( Error_Status )
    ...
    Error_Status = NESDIS_WaterIR_Emiss(Wind_Speed,       &
                                        Wavenumber,       &
                                        Angle(1:nZ),      &
                                        Emissivity(1:nZ), &
                                        iVar%IRSSEM )
  ...
  END FUNCTION Compute_WaterIR_Emiss
...
!
! CALLING SEQUENCE:
!       Error_Status = Compute_WaterIR_Emiss_BRDF( &
!                        Wavenumber             , &  ! Input
!                        Wind_Speed             , &  ! Input
!                        Angle                  , &  ! Input
...
!----------------------------------------------------------------------------------
  ...
  FUNCTION Compute_WaterIR_Emiss_BRDF( &
      Wavenumber                    , &  ! Input
      Wind_Speed                    , &  ! Input
      Angle                         , &  ! Input
      Sensor_Zenith_Radian          , &  ! Input
  ...
  END FUNCTION Compute_WaterIR_Emiss_BRDF
...
!
! CALLING SEQUENCE:
!       Error_Status = NESDIS_IRSSEM_BRDF_TL(      &
!                        Wind_Speed_TL           , &  ! Input
...
!----------------------------------------------------------------------------------
  ...
  FUNCTION NESDIS_IRSSEM_BRDF_TL( &
    Wind_Speed_TL               , &  ! Input
    Emissivity_TL               , &  ! Output
    Reflectivity_TL             , &  ! Output
    Direct_Reflectivity_TL      , &  ! output
  ...
  RESULT ( Error_Status )
    ...
    Error_Status = NESDIS_WaterIR_Emiss_TL(Wind_Speed_TL, &
                                           Emissivity_TL(1:nZ), &
                                           iVar%IRSSEM )
    IF ( Error_Status /= SUCCESS ) THEN
      CALL Display_Message( ROUTINE_NAME, &
  ...
  END FUNCTION NESDIS_IRSSEM_BRDF_TL
...
!
! CALLING SEQUENCE:
!       Error_Status = NESDIS_IRSSEM_BRDF_AD(      &
...
!----------------------------------------------------------------------------------
  ...
  FUNCTION NESDIS_IRSSEM_BRDF_AD( &
    Emissivity_AD               , &  ! Input
  ...
  RESULT ( Error_Status )
    ...
    Error_Status = NESDIS_WaterIR_Emiss_AD( Emissivity_AD(1:nZ), &
                                           Wind_Speed_AD,        &
                                           iVar%IRSSEM )
    IF ( Error_Status /= SUCCESS ) THEN
      CALL Display_Message( ROUTINE_NAME, &
  ...
  END FUNCTION NESDIS_IRSSEM_BRDF_AD

  FUNCTION NESDIS_IRSSEM_Setup( File_Name)  RESULT( Error_Status )
    ...
    IF (PRESENT(File_Name) ) THEN
        Error_Status=IRSSEM_Setup(TRIM(File_Name))
    ELSE     
        Error_Status = IRSSEM_Setup()
    END IF    


  END FUNCTION NESDIS_IRSSEM_Setup
  
  SUBROUTINE NESDIS_IRSSEM_Close( )
    INTEGER :: Error_Status
    IF (IRSSEM_Initialized()) THEN
      Error_Status = IRSSEM_CleanUP()
    ENDIF
   
  END SUBROUTINE NESDIS_IRSSEM_Close
  
  FUNCTION NESDIS_IRSSEM_Initialized() RESULT( LUT_Status )
    LOGICAL :: LUT_Status
    LUT_Status=IRSSEM_Initialized() 
  END FUNCTION NESDIS_IRSSEM_Initialized

END MODULE NESDIS_WaterIR_PhyModel
