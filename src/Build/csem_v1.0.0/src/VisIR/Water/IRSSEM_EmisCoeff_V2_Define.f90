!
! IRSSEM_EmisCoeff_V2_Define
!
! Module defining the V2 EmisCoeff data structure and containing
! routines to manipulate it.
!
! It is modified from the V1 EmisCoeff.
!
!
! CREATION HISTORY:
...
!
!       Modified by:    Ming Chen, ESSIC_UMD, 13-Apr-2020
!                       ming.chen@noaa.gov

MODULE IRSSEM_EmisCoeff_V2_Define

  ! ------------------
  ! Environment set up
  ! ------------------
  ! Module use
  USE CSEM_Type_Kinds, ONLY: Long   => CSEM_Long, &
                             Double => CSEM_Double
  USE CSEM_Exception_Handler,         ONLY: SUCCESS, FAILURE, Display_Message
  USE CSEM_Compare_Float_Numbers, ONLY: Compare_Float
  ...
  PRIVATE
  ! Parameters
  PUBLIC :: N_EMISCOEFF_V2_ITEMS
  PUBLIC :: EMISCOEFF_V2_DATA_TYPE
  PUBLIC :: EMISCOEFF_V2_DATA_NAME
  PUBLIC :: SPECTRAL_EMISCOEFF_V2_TYPE
  PUBLIC :: SENSOR_EMISCOEFF_V2_TYPE
  ! Datatypes
  PUBLIC :: EmisCoeff_V2_type
  ! Procedures
  PUBLIC :: Associated_EmisCoeff_V2
  PUBLIC :: Destroy_EmisCoeff_V2
  PUBLIC :: Allocate_EmisCoeff_V2
  PUBLIC :: Assign_EmisCoeff_V2
  PUBLIC :: Equal_EmisCoeff_V2
  PUBLIC :: Check_EmisCoeff_V2_Release
  PUBLIC :: Info_EmisCoeff_V2
  ...
  '$Id: EmisCoeff_Define.f90 14438 2011-06-30 12:46:05Z david.groff@noaa.gov $'
  ...
  INTEGER(Long), PARAMETER :: SPECTRAL_EMISCOEFF_V2_TYPE = 1
  INTEGER(Long), PARAMETER :: SENSOR_EMISCOEFF_V2_TYPE   = 2
  ! Number of EmisCoeff data items
  INTEGER(Long), PARAMETER :: N_EMISCOEFF_V2_ITEMS = 5_Long
  ...
  INTEGER(Long), PARAMETER, DIMENSION( N_EMISCOEFF_V2_ITEMS ) :: &
  EMISCOEFF_V2_DATA_TYPE = (/ DOUBLE_TYPE, &  ! Angle
                             DOUBLE_TYPE, &  ! Frequency
                             DOUBLE_TYPE, &  ! Wind speed
                             DOUBLE_TYPE, &  ! Temperature
                             DOUBLE_TYPE /)  ! Emissivity
  ! Names of the data items (for error processing)
  CHARACTER(*), PARAMETER, DIMENSION( N_EMISCOEFF_V2_ITEMS ) :: &
    EMISCOEFF_V2_DATA_NAME = (/ 'Angle      ', &
                             'Frequency  ', &
                             'Wind speed ', &
                             'Temperature', &
                             'Emissivity ' /)
  ...
  TYPE :: EmisCoeff_V2_type
    ...
    REAL(Double), POINTER :: Temperature(:) => NULL()  ! M
    ! The spectral emissivity
    REAL(Double), POINTER :: Emissivity(:,:,:,:) => NULL()  ! I x L x N x M
  END TYPE EmisCoeff_V2_type
...
!
! CALLING SEQUENCE:
!       CALL Clear_EmisCoeff_V2( EmisCoeff ) ! Output
!
! OUTPUT ARGUMENTS:
...
!----------------------------------------------------------------------------------
  ...
  SUBROUTINE Clear_EmisCoeff_V2( EmisCoeff )
    TYPE(EmisCoeff_V2_type), INTENT(IN OUT) :: EmisCoeff
    EmisCoeff%Release = EMISCOEFF_RELEASE 
    EmisCoeff%Version = EMISCOEFF_VERSION 
  END SUBROUTINE Clear_EmisCoeff_V2
...
!
! CALLING SEQUENCE:
!       Association_Status = Associated_EmisCoeff_V2( EmisCoeff,          &  ! Input
...
!--------------------------------------------------------------------------------
  ...
  FUNCTION Associated_EmisCoeff_V2( EmisCoeff, & ! Input
                                 ANY_Test ) & ! Optional input
                               RESULT( Association_Status )
    ! Arguments
    TYPE(EmisCoeff_V2_type), INTENT(IN) :: EmisCoeff
    ...
    IF ( ALL_Test ) THEN
      IF ( ASSOCIATED( EmisCoeff%Angle      ) .AND. &
           ...
           ASSOCIATED( EmisCoeff%Temperature ) .AND. &
    ...
    ELSE
      IF ( ASSOCIATED( EmisCoeff%Angle      ) .OR. &
           ...
           ASSOCIATED( EmisCoeff%Temperature ) .OR. &
           ASSOCIATED( EmisCoeff%Emissivity )      ) THEN
        Association_Status = .TRUE.
      END IF
    END IF

  END FUNCTION Associated_EmisCoeff_V2
...
!
! CALLING SEQUENCE:
!       Error_Status = Destroy_EmisCoeff_V2( EmisCoeff,                &  ! Output
...
!------------------------------------------------------------------------------
  ...
  FUNCTION Destroy_EmisCoeff_V2( EmisCoeff,    &  ! Output
    ...
    END IF
    IF ( Clear ) CALL Clear_EmisCoeff_v2( EmisCoeff )

    ! If ALL pointer members are NOT associated, do nothing
    IF ( .NOT. Associated_EmisCoeff_V2( EmisCoeff ) ) RETURN
  ...
  END FUNCTION Destroy_EmisCoeff_V2
...
!
! CALLING SEQUENCE:
!       Error_Status = Allocate_EmisCoeff_V2( n_Angles,               &  ! Input
!                                          n_Frequencies,             &  ! Input
!                                          n_Wind_Speeds,             &  ! Input
...
!                      ATTRIBUTES: INTENT(IN)
!
!       n_Temperatures: Number of sst dimension.
!                      Must be > 0.
!                      UNITS:      N/A
!                      TYPE:       INTEGER
!                      DIMENSION:  Scalar
...
!------------------------------------------------------------------------------
  ...
  FUNCTION Allocate_EmisCoeff_V2( n_Angles,      &  ! Input
                               n_Frequencies, &  ! Input
                               n_Wind_Speeds, &  ! Input
                               n_Temperatures, &  ! Input
                               EmisCoeff,     &  ! Output
    ...
    IF ( Associated_EmisCoeff_V2( EmisCoeff, ANY_Test=SET ) ) THEN
      Error_Status = Destroy_EmisCoeff_V2( EmisCoeff, &
                                        No_Clear=SET, &
                                        Message_Log=Message_Log )
    ...
    ALLOCATE( EmisCoeff%Angle( n_Angles ), &
              ...
              EmisCoeff%Temperature( n_Temperatures ), &
              EmisCoeff%Emissivity( n_Angles, n_Frequencies, n_Wind_Speeds, n_temperatures ), &
              STAT = Allocate_Status )
    IF ( Allocate_Status /= 0 ) THEN
      Error_Status = FAILURE
    ...
    EmisCoeff%n_Wind_Speeds = n_Temperatures
  ...
  END FUNCTION Allocate_EmisCoeff_V2
...
!
! CALLING SEQUENCE:
!       Error_Status = Assign_EmisCoeff_V2( EmisCoeff_in,             &  ! Input
...
!------------------------------------------------------------------------------
  ...
  FUNCTION Assign_EmisCoeff_V2( EmisCoeff_in,  &  ! Input
                             EmisCoeff_out, &  ! Output
                             RCS_Id,        &  ! Revision control
                             Message_Log )  &  ! Error messaging
                           RESULT( Error_Status )
    ! Arguments
    TYPE(EmisCoeff_V2_type),   INTENT(IN)     :: EmisCoeff_in
    TYPE(EmisCoeff_V2_type),   INTENT(IN OUT) :: EmisCoeff_out
    CHARACTER(*), OPTIONAL,    INTENT(OUT)    :: RCS_Id
    CHARACTER(*), OPTIONAL,    INTENT(IN)     :: Message_Log
    ! Function result
    INTEGER :: Error_Status
    ...
    IF ( .NOT. Associated_EmisCoeff_V2( EmisCoeff_In ) ) THEN
      Error_Status = FAILURE
      CALL Display_Message( ROUTINE_NAME,    &
    ...
    ! ----------------------
    Error_Status = Allocate_EmisCoeff_V2( EmisCoeff_in%n_Angles     , &
                                       EmisCoeff_in%n_Frequencies, &
                                       EmisCoeff_in%n_Wind_Speeds, &
                                       EmisCoeff_in%n_Temperatures, &
                                       EmisCoeff_out, &
                                       Message_Log = Message_Log )
    IF ( Error_Status /= SUCCESS ) THEN
      CALL Display_Message( ROUTINE_NAME,    &
    ...
    EmisCoeff_out%Temperature = EmisCoeff_in%Temperature
    EmisCoeff_out%Emissivity = EmisCoeff_in%Emissivity

  END FUNCTION Assign_EmisCoeff_V2
...
!
! CALLING SEQUENCE:
!       Error_Status = Equal_EmisCoeff_V2( EmisCoeff_LHS          , &  ! Input
!                                       EmisCoeff_RHS          , &  ! Input
...
!------------------------------------------------------------------------------
  ...
  FUNCTION Equal_EmisCoeff_V2( EmisCoeff_LHS, &  ! Input
                            EmisCoeff_RHS, &  ! Input
                          ...
                          RESULT( Error_Status )
    ! Arguments
    TYPE(EmisCoeff_V2_type),   INTENT(IN)  :: EmisCoeff_LHS
    TYPE(EmisCoeff_V2_type),   INTENT(IN)  :: EmisCoeff_RHS
    ...
    IF ( .NOT. Associated_EmisCoeff_V2( EmisCoeff_LHS ) ) THEN
      Error_Status = FAILURE
      CALL Display_Message( ROUTINE_NAME, &
      ...
      RETURN
    END IF
    IF ( .NOT. Associated_EmisCoeff_V2( EmisCoeff_RHS ) ) THEN
      Error_Status = FAILURE
      CALL Display_Message( ROUTINE_NAME,    &
    ...
    IF ( EmisCoeff_LHS%n_Angles      /= EmisCoeff_RHS%n_Angles      .OR. &
         ...
         EmisCoeff_LHS%n_Temperatures /= EmisCoeff_RHS%n_Temperatures .OR. &
    ...
    ! -------------------------
    DO m = 1, EmisCoeff_LHS%n_Temperatures
    DO n = 1, EmisCoeff_LHS%n_Wind_Speeds
      DO l = 1, EmisCoeff_LHS%n_Frequencies
        DO i = 1, EmisCoeff_LHS%n_Angles
          IF ( .NOT. Compare_Float( EmisCoeff_LHS%Emissivity(i,l,n,m), &
                                    EmisCoeff_RHS%Emissivity(i,l,n,m), &
                                    ULP = ULP                ) ) THEN
            Error_Status = FAILURE
            WRITE( Message,'("Emissivity values are different at (",i0,",",i0,",",i0,")",&
                            &":",3(1x,es13.6))') &
                           i,l,n, &
                           EmisCoeff_LHS%Emissivity(i,l,n,m),EmisCoeff_LHS%Emissivity(i,l,n,m), &
                           EmisCoeff_LHS%Emissivity(i,l,n,m)-EmisCoeff_LHS%Emissivity(i,l,n,m)
            CALL Display_Message( ROUTINE_NAME, &
                                  TRIM(Message), &
                                  Error_Status, &
  ...
  END FUNCTION Equal_EmisCoeff_V2
...
!
! NAME:
!       Check_EmisCoeff_V2_Release
!
! PURPOSE:
!       Function to check the EmisCoeff Release value.
!
! CALLING SEQUENCE:
!       Error_Status = Check_EmisCoeff_V2_Release( EmisCoeff,                &  ! Input
!                                               RCS_Id      = RCS_Id,     &  ! Revision control
!                                               Message_Log = Message_Log )  ! Error messaging
!
! INPUT ARGUMENTS:
...
!----------------------------------------------------------------------------------
  ...
  FUNCTION Check_EmisCoeff_V2_Release( EmisCoeff,    &  ! Input
                                    RCS_Id,       &  ! Revision control
                                    Message_Log ) &  ! Error messaging
                                    RESULT( Error_Status )
    ! Arguments
  ...
  END FUNCTION Check_EmisCoeff_V2_Release
...
!
! CALLING SEQUENCE:
!       CALL Info_EmisCoeff_V2( EmisCoeff,      &  ! Input
!                            Info,           &  ! Output
!                            RCS_Id = RCS_Id )  ! Revision control
!
! INPUT ARGUMENTS:
...
!------------------------------------------------------------------------------
  ...
  SUBROUTINE Info_EmisCoeff_V2( EmisCoeff, &  ! Input
                             Info,      &  ! Output
                             RCS_Id     )  ! Revision control
    ! Arguments
    TYPE(EmisCoeff_V2_type),   INTENT(IN)  :: EmisCoeff
  ...
  END SUBROUTINE Info_EmisCoeff_V2

END MODULE IRSSEM_EmisCoeff_V2_Define
