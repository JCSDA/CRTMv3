...
!
! This module is modified from the CRTM_IR_Water_SfcOptics module which was 
! written by Paul van Delst, 25-Jun-2005. ONLY ocean BRDF model is packed in 
! this dedicated module. 
!
!
! CREATION HISTORY:
!       Written by:     Ming Chen, Feb-15-2021
!                       ming.chen@noaa.gov
!

MODULE NESDIS_WaterVIS_BRDF_Module
  ...
  PUBLIC :: iVar_type
  ! Science routines
  PUBLIC :: NESDIS_VISWater_BRDF
  PUBLIC :: NESDIS_VISWater_BRDF_TL
  PUBLIC :: NESDIS_VISWater_BRDF_AD
  ...
  TYPE :: iVar_type
    ...
    LOGICAL  :: IsSolar
    REAL(fp) :: Wind_Speed     ! Wind_Speed
    REAL(fp) :: Source_Zenith_Radian     ! Wind_SpeedSource_Zenith_Radian
  END TYPE iVar_type
...
!################################################################################
  ...
  RESULT( Error_Status )
    ...
    ! Local parameters
    CHARACTER(*), PARAMETER :: ROUTINE_NAME = 'Compute_VISWater_DRDF'
    ! Local variables
  ...
  END FUNCTION NESDIS_VISWater_BRDF

  FUNCTION NESDIS_VISWater_BRDF_TL( &
    Wind_Speed_TL           , &  ! Input
  ...
  RESULT( Error_Status )
    ...
    ! Set up
    Error_Status = SUCCESS
    IF(iVar%Source_Zenith_Radian < PI/TWO ) THEN
        CALL BRDF_Rough_Sea_TL(iVar%Wind_Speed,                &
                               Wind_Speed_TL,                  &
                               brdf_TL,                        &
                               iVar)
        Direct_Reflectivity_TL(:) = brdf_TL
    ELSE
        Direct_Reflectivity_TL = ZERO
    END IF
 

  END FUNCTION NESDIS_VISWater_BRDF_TL

  FUNCTION NESDIS_VISWater_BRDF_AD( &
  ...
  RESULT ( Error_Status )
    ...
    Error_Status = SUCCESS
    IF(iVar%Source_Zenith_Radian < PI/TWO ) THEN
        brdf_AD = SUM(Direct_Reflectivity_AD)
        Direct_Reflectivity_AD = ZERO
        CALL BRDF_Rough_Sea_AD(iVar%Wind_Speed,               &
                               brdf_AD,                       &
                               Wind_Speed_AD,                 &
                               iVar)
    ELSE
  ...
  END FUNCTION NESDIS_VISWater_BRDF_AD
...
!##################################################################################
  ...
  SUBROUTINE BRDF_Rough_Sea(Frequency, theta_s, dphi, theta_r, Wind_Speed, &
    ...
    iVar%W       = PI * rho * sec4_theta_f / (4.0_fp*cos_theta_r*cos_theta_s)
  ...
  FUNCTION Ref_Index(Frequency) RESULT(ref)
    ...
    !-------------------------------------------------------------------------
    ! added refractive indices for visible from old CRTM, Q. Liu, July 3, 2019
    !
     INTEGER, PARAMETER  :: nf = 196    !151
    ...
    REAL(fp), PARAMETER :: freq(nf) = (/ &
     ...
     3500.0,  3571.4, 3636.4, 3703.7, 3773.6, 3846.2, 4166.7, &
     4545.5, 5000.0, 5555.6, 6250.0, 7142.9, 8333.3,10000.0,10256.4,10526.3,10810.8, &
    11111.1,11428.6,11764.7,12121.2,12500.0,12903.2,13333.3,13793.1,14285.7,14814.8, &
    15384.6,16000.0,16666.7,17391.3,18181.8,19047.6,20000.0,21052.6,22222.2,23529.4, &
    25000.0,26666.7,28571.4,30769.2,33333.3,36363.6,40000.0,44444.4,50000.0/)
    ...
    REAL(fp), PARAMETER :: nr(nf) = (/ &
     ...
     1.1410, 1.1420, 1.1570, 1.1880, 1.2190, 1.2420, 1.2790, &
     1.2960, 1.3060, 1.3120, 1.3170, 1.3210, 1.3240, 1.3270, 1.3270, 1.3270, 1.3280, &
     1.3280, 1.3280, 1.3290, 1.3290, 1.3290, 1.3300, 1.3300, 1.3300, 1.3310, 1.3310, &
     1.3310, 1.3320, 1.3320, 1.3330, 1.3330, 1.3340, 1.3350, 1.3360, 1.3370, 1.3380, &
     1.3390, 1.3410, 1.3430, 1.3460, 1.3490, 1.3540, 1.3620, 1.3730, 1.3960/)
    ...
    REAL(fp), PARAMETER :: ni(nf) = (/ &
     ...
     0.2230, 0.1150, 0.0590, 0.0190, 0.0067, 0.0032, 0.0010, &
     0.0003, 0.0011, 0.0001, 0.0001, 0.0001, 0.0000, 0.0000, 0.0000, 0.0000, 0.0000, &
...
!      idx = INT((Frequency - freq(1))/df) + 1  ! find the starting index
      idx = Freq_search(Frequency, freq, nf )
  ...
  FUNCTION Freq_search(Frequency, freq, nf) RESULT(idx)
    REAL(fp), INTENT(IN) :: Frequency, freq(:)
    INTEGER :: idx, nf, i

    IF( Frequency >= freq(nf) ) THEN
      idx = nf-1
    ELSE
      DO i = 1, nf-1
        IF( Frequency < freq(i) ) GO TO 201
      END DO
 201  idx = i-1
    END IF
    
  END FUNCTION Freq_search


END MODULE NESDIS_WaterVIS_BRDF_module
