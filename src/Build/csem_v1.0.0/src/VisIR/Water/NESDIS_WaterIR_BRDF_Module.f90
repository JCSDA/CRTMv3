...
MODULE NESDIS_WaterIR_BRDF_Module
  ...
  PUBLIC :: iVar_type
  ! Science routines
  PUBLIC :: NESDIS_IRWater_BRDF
  PUBLIC :: NESDIS_IRWater_BRDF_TL
  PUBLIC :: NESDIS_IRWater_BRDF_AD
...
!################################################################################
  ...
  RESULT( Error_Status )
    ...
    ! Local parameters
    CHARACTER(*), PARAMETER :: ROUTINE_NAME = 'Compute_IRWater_DRDF'
    ! Local variables
  ...
  END FUNCTION NESDIS_IRWater_BRDF

  FUNCTION NESDIS_IRWater_BRDF_TL( &
    Wind_Speed_TL           , &  ! Input
  ...
  END FUNCTION NESDIS_IRWater_BRDF_TL

  FUNCTION NESDIS_IRWater_BRDF_AD( &
  ...
  END FUNCTION NESDIS_IRWater_BRDF_AD
  ...
  FUNCTION Ref_Index(Frequency) RESULT(ref)
    ...
    ELSE
      ...
      ref = CMPLX( nr(idx) + c*(nr(idx+1) - nr(idx)), &
                   ni(idx) + c*(ni(idx+1) - ni(idx)),fp )
    END IF

  END FUNCTION Ref_Index

END MODULE NESDIS_WaterIR_BRDF_module
