...
MODULE CRTM_LowFrequency_MWSSEM
  ...
  USE FASTEM_Fresnel    , ONLY: fVar_type => iVar_type , &
  ...
  USE CRTM_FASTEM_Parameters   , ONLY: PI, DEGREES_TO_RADIANS, &
  ...
  '$Id: CRTM_LowFrequency_MWSSEM.f90 48006 2014-11-24 12:06:32Z paul.vandelst@noaa.gov $'
  ...
  TYPE :: iVar_type
    ...
    TYPE(CSEM_LPoly_type) :: flp  ! Frequency
    TYPE(CSEM_LPoly_type) :: wlp  ! Wind speed
...
!--------------------------------------------------------------------------------
  ...
  SUBROUTINE LowFrequency_MWSSEM( Frequency   , &  ! Input
    ...
    CALL CSEM_Find_Index(FREQUENCY_SDD, D_FREQUENCY, iVar%f_int, iVar%i1, iVar%i2, f_outbound)

    iVar%w_int = MAX(MIN(WIND_SPEED_SDD(N_WIND_SPEEDS),Wind_Speed),WIND_SPEED_SDD(1))
    CALL CSEM_Find_Index(WIND_SPEED_SDD, D_WIND_SPEED, iVar%w_int, iVar%j1, iVar%j2, w_outbound)


    ! Calculate the interpolating polynomials
    ! ...Frequency term
    CALL CSEM_LPoly( FREQUENCY_SDD(iVar%i1:iVar%i2), &  ! Input
                iVar%f_int                    , &  ! Input
                iVar%flp                        )  ! Output
    ! ...Wind speed term
    CALL CSEM_LPoly( WIND_SPEED_SDD(iVar%j1:iVar%j2), &  ! Input
                iVar%w_int                     , &  ! Input
                iVar%wlp                         )  ! Output


    ! Perform the 2-D interpolation
    CALL CSEM_Interp_2D( sdd(iVar%i1:iVar%i2, iVar%j1:iVar%j2), &
...
!--------------------------------------------------------------------------------
  ...
  SUBROUTINE LowFrequency_MWSSEM_TL( &
    ...
    TYPE(CSEM_LPoly_type) :: flp_TL, wlp_TL
    ...
    CALL CSEM_Clear_LPoly(flp_TL)
    ! Wind speed term
    CALL CSEM_LPoly_TL( WIND_SPEED_SDD(iVar%j1:iVar%j2), &  ! FWD Input
    ...
    CALL CSEM_Interp_2D_TL( sdd(iVar%i1:iVar%i2, iVar%j1:iVar%j2), &  ! FWD Input
...
!--------------------------------------------------------------------------------
  ...
  SUBROUTINE LowFrequency_MWSSEM_AD( &
    ...
    TYPE(CSEM_LPoly_type) :: flp_AD, wlp_AD
    ...
    CALL CSEM_Interp_2D_AD( sdd(iVar%i1:iVar%i2, iVar%j1:iVar%j2), &  ! FWD Input
                       iVar%flp, iVar%wlp,                    &  ! FWD Input
                       sdd_int_AD,                            &  ! AD  Input
                       sdd_AD,                                &  ! AD  Output
    ...
    ! ...Wind speed term
    CALL CSEM_LPoly_AD( WIND_SPEED_SDD(iVar%j1:iVar%j2), &  ! FWD Input
                   iVar%w_int                     , &  ! FWD Input
                   iVar%wlp                       , &  ! FWD Input
                   wlp_AD                         , &  ! AD  Input
                   w_AD                           , &  ! AD  Input
                   w_int_AD                         )  ! AD  Output
    ! ...No frequency term CSEM_LPoly_AD.
