...
! CSEM_TELSEM_Atlas
!
! This module is provided to allow users to use TELSEM land surface emissivity data sets by
! CSEM interfaces.
! TELSEM includes the monthly land surface emissivity atlas based on the multiple-year
! retrievals from SSMI and some trievals from other sensors. TELSEM is a generalized atlas
...
MODULE TELSEM_Atlas_Module
  ...
  LOGICAL,SAVE :: telsem_atlas_init
  INTEGER,SAVE :: telsem_atlas_month
  
  PUBLIC :: TELSEM_Atlas_Initialized
  PUBLIC :: TELSEM_Atlas_Setup
  PUBLIC :: TELSEM_Atlas_Emiss
  PUBLIC :: TELSEM_Atlas_Emiss_nChannels
  PUBLIC :: TELSEM_Atlas_Close

CONTAINS

  FUNCTION TELSEM_Atlas_Setup( &
               &  imonth,       &! in
               &  path,         &! in, optional
    ...
    CHARACTER(*), PARAMETER :: ROUTINE_NAME = 'CSEM_TELSEM_Atlas_Setup'
    ! Local variables
    ...
    IF (telsem_atlas_init) THEN
        IF(imonth == telsem_atlas_month) THEN
    ...
    Error_Status=load_telsem_atlas(TRIM(fpath),imonth)
    !PRINT*,'Initialising TELSEM Atlas of month ', imonth
    IF (Error_Status /= SUCCESS) THEN
        Error_Status = FAILURE
       PRINT*, 'Error initialising MW emissivity atlas TELSEM ...'
       RETURN
    END IF
    telsem_atlas_month = imonth
    telsem_atlas_init = .TRUE.

  END FUNCTION TELSEM_Atlas_Setup
...
!
!       This function is dedicated to using TELSEM monthly atals.
!
! CALLING SEQUENCE:
!       Error_Status = TELSEM_Atlas_Emiss(    &
!                         Frequency,             & ! input
...
!----------------------------------------------------------------------------------
  ...
  FUNCTION TELSEM_Atlas_Emiss(  &
    ...
    CHARACTER(*), PARAMETER :: ROUTINE_NAME = 'TELSEM_Atlas_Emiss'
    ...
    IF(.NOT. TELSEM_Atlas_Initialized(imonth)) THEN
      PRINT*,'TELSEM Atlas has not been initilizaed for the month ', imonth
      Error_Status=TELSEM_Atlas_Setup(imonth)
    ENDIF
  ...
  END FUNCTION TELSEM_Atlas_Emiss

!----------------------------------------------------------------------------------
!:sdoc+:
!
! NAME:
!       TELSEM_Atlas_Emiss_nChannels
!
! PURPOSE:
...
!----------------------------------------------------------------------------------
  ...
  FUNCTION TELSEM_Atlas_Emiss_nChannels(   &
     & Frequency,                          & ! input
     & Angle,                              & ! input
     & Latitude,                           & ! input
     & Longitude,                          & ! input
    ...
    !CHARACTER(LEN=100) :: Message
    CHARACTER(*), PARAMETER :: ROUTINE_NAME = 'TELSEM_Atlas_Emiss_nChannels'
    INTEGER(KIND=1) :: input_type = 0
  ...
  END FUNCTION TELSEM_Atlas_Emiss_nChannels
  ...
  SUBROUTINE TELSEM_Atlas_Close( )
    IF (telsem_atlas_init) THEN
      PRINT*,'Clean TELSEM Atlas ...'
      CALL close_telsem_atlas
      telsem_atlas_init = .FALSE.
    ENDIF
   
  END SUBROUTINE TELSEM_Atlas_Close
  
  FUNCTION TELSEM_Atlas_Initialized(imonth) RESULT( Atlas_Status )
     INTEGER :: imonth 
     LOGICAL :: Atlas_Status
     Atlas_Status=(telsem_atlas_init .AND. (imonth == telsem_atlas_month)) 
  END FUNCTION TELSEM_Atlas_Initialized
END MODULE TELSEM_Atlas_Module
