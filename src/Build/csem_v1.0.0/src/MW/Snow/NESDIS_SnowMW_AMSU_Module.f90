...
MODULE NESDIS_AMSU_SnowEM_Module
  ...
  '$Id: NESDIS_AMSU_SnowEM_Module.f90 21141 2012-09-14 17:40:43Z paul.vandelst@noaa.gov $'
...
!----------------------------------------------------------------------------------------------------------!
  ...
  real(fp)    :: kratio, bconst,emissivity=1.0_fp
  integer :: ich
...
!   using the NESDIS_MWEmiss_Snow and a bias correction algorithm, where the original NESDIS_MWEmiss_Snow with a
!   bias correction algorithm is referred to as value-added NESDIS_MWEmiss_Snow or AlandEM.
!
! program history log:
!
! input argument list:
...
!                           at three AMSU window channels computed using NESDIS_MWEmiss_Snow
...
!------------------------------------------------------------------------------------------------------------
  ...
  call NESDIS_MWEmiss_Snow(theta, frequency,&
        0.0_fp,0.0_fp,ts,ts,0.0_fp,9,13,snow_depth,esh0,esv0)

  theta0 = theta
  do ich = 1, nw_ind
     freq =freq_3w(ich)
     theta = theta0
     call NESDIS_MWEmiss_Snow(theta, freq,&
        0.0_fp,0.0_fp,ts,ts,0.0_fp,9,13,snow_depth,esh,esv)
     esv_3w(ich) = esv
     esh_3w(ich) = esh
  end do
