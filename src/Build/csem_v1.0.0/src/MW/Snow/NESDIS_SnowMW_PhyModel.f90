...
MODULE NESDIS_SnowMW_PhyModel
  ...
  REAL(fp), PARAMETER :: C_2 = 1.438775246065196E-002_fp   ! Second Planck function constant
                                                   ! c2=h.c/k
...
!
! PURPOSE:
!       Function to simulate microwave emissivity over snow surface conditions.
!
! REFERENCES:
...
!
! CALLING SEQUENCE:
!              Error_Status = NESDIS_SnowMW_Emiss(              &
!                                  Frequency,                   &  ! Input
!                                  Angle,                       &  ! Input
...
!
! OUTPUT ARGUMENTS:
!         Emissivity_H:           Snow emissivity at a horizontal
!                                  polarization.
!                                  UNITS:      N/A
!                                  TYPE:       REAL(fp)
!                                  DIMENSION:  Scalar
!
!         Emissivity_V:            Snow emissivity at a vertical polarization.
!                                  UNITS:      N/A
!                                  TYPE:       REAL(fp)
!                                  DIMENSION:  Scalar
!
...
!------------------------------------------------------------------------------------------------------------
  ...
  SUBROUTINE NESDIS_SnowMW_Emiss(Angle,           &   ! Input
                           Frequency,             &   ! Input
                           Soil_Moisture_Content, &   ! Input
...
!     leaf_thick = 0.07_fp
  ...
  END SUBROUTINE NESDIS_SnowMW_Emiss

 SUBROUTINE NESDIS_LandEM_OLD(Angle,                 &   ! Input
                           Frequency,             &   ! Input
                           Soil_Moisture_Content, &   ! Input
                           Vegetation_Fraction,   &   ! Input
                           Soil_Temperature,      &   ! Input
...
!     leaf_thick = 0.07_fp
  ...
  END SUBROUTINE NESDIS_LandEM_OLD
...
subroutine SnowEM_Default2(frequency,ts, depth,Emissivity_V,Emissivity_H)
...
!------------------------------------------------------------------------------------------------------------
  ...
  frequency=frequency
  ts = ts; depth = depth
  Emissivity_V = EMISSH_DEFAULT
  Emissivity_H = EMISSH_DEFAULT

end subroutine SnowEM_Default2
...
END MODULE NESDIS_SnowMW_phyModel
