...
MODULE NESDIS_SSMI_ICEEM_Module
  ...
  ! ------------
  PRIVATE
  PUBLIC :: NESDIS_SSMI_IceEM

  REAL(fp), PARAMETER :: ONE = 1.0_fp, ZERO = 0.0_fp, ONE_TENTH=0.1_fp
  ...
  '$Id: NESDIS_SSMI_SIceEM_Module.f90 21141 2012-09-14 17:40:43Z paul.vandelst@noaa.gov $'
...
subroutine NESDIS_SSMI_IceEM(frequency,                                          & ! INPUT
                               Angle,                                             & ! INPUT
                               Ts,                                                & ! INPUT
                               tb,                                                & ! INPUT
                               Depth,                                             & ! INPUT
...
! Get the emissivity angle dependence
  ...
  call NESDIS_MWEmiss_Ice(SSMI_Angle,frequency,&
      0.0_fp,0.0_fp,Ts,Ts,0.0_fp,9,13,Depth,esh1,esv1)

  call NESDIS_MWEmiss_Ice(Angle,frequency,&
      0.0_fp,0.0_fp,Ts,Ts,0.0_fp,9,13,Depth,esh2,esv2)
...
end subroutine NESDIS_SSMI_IceEM
...
END MODULE NESDIS_SSMI_ICEEM_Module
