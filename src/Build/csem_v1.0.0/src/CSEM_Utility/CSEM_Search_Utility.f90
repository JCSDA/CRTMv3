...
MODULE CSEM_Search_Utility
  ...
  ! ------------
  PRIVATE
  PUBLIC :: CSEM_Value_Locate
  PUBLIC :: CSEM_Bisection_Search
  ...
  CHARACTER(*), PARAMETER :: MODULE_VERSION_ID = &
    '$Id: Search_Utility.f90 14489 2011-07-06 17:46:47Z paul.vandelst@noaa.gov $'
...
!
! CALLING SEQUENCE:
!       j = CSEM_Bisection_Search( x, u,          &  ! Input
...
!------------------------------------------------------------------------------
  ...
  FUNCTION CSEM_Bisection_Search( x, u, xLower, xUpper ) RESULT( j )
    ! Arguments
  ...
  END FUNCTION CSEM_Bisection_Search
...
!------------------------------------------------------------------------------
  ...
  FUNCTION CSEM_Value_Locate( x, u ) RESULT( j )
    ! Arguments
    REAL(fp), DIMENSION(:), INTENT(IN) :: x
    ...
    IF ( ascending ) THEN
      ...
      xLower = 1
      DO iu = 1, nu
        j(iu) = CSEM_Bisection_Search( x, u(iu), xLower = xLower )
        xLower = MAX( 1, j(iu) )
      END DO
    ELSE
      ! Going down
      xUpper = nx
      DO iu = 1, nu
        j(iu) = CSEM_Bisection_Search( x, u(iu), xUpper = xUpper )
        xLower = MIN( j(iu), nx )
      END DO
    END IF

  END FUNCTION CSEM_Value_Locate

END MODULE CSEM_Search_Utility
