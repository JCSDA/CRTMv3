...
! routines used in the CSEM
! It is copied from the CRTM module file CRTM_Interpolation.f90.
! The module name and the variable names are modified to avoid 
! namespace conflicts when CSEM coupled with CRTM.
!
!
! CREATION HISTORY:
...
!
!       Modified by:    Ming Chen 08-May-2016
!                       Ming.Chen@noaa.gov
!
MODULE CSEM_Interpolation
  ...
  PUBLIC :: CSEM_LPoly_type
  PUBLIC :: CSEM_Clear_LPoly
  PUBLIC :: CSEM_LPoly_Init
  PUBLIC :: CSEM_LPoly_Inspect
  ! Procedures
  PUBLIC :: CSEM_Interp_1D
  PUBLIC :: CSEM_Interp_2D
  PUBLIC :: CSEM_Interp_3D
  PUBLIC :: CSEM_Interp_4D
  PUBLIC :: CSEM_Interp_1D_TL
  PUBLIC :: CSEM_Interp_2D_TL
  PUBLIC :: CSEM_Interp_3D_TL
  PUBLIC :: CSEM_Interp_4D_TL
  PUBLIC :: CSEM_Interp_1D_AD
  PUBLIC :: CSEM_Interp_2D_AD
  PUBLIC :: CSEM_Interp_3D_AD
  PUBLIC :: CSEM_Interp_4D_AD
  PUBLIC :: CSEM_Find_Index
  PUBLIC :: CSEM_LPoly
  PUBLIC :: CSEM_LPoly_TL
  PUBLIC :: CSEM_LPoly_AD


  ! -------------------
  ! Procedure overloads
  ! -------------------
  INTERFACE CSEM_Find_Index
    MODULE PROCEDURE Find_Regular_Index
    MODULE PROCEDURE Find_Random_Index
  END INTERFACE CSEM_Find_Index
  ...
  '$Id: CSEM_Interpolation.f90 21141 2012-09-14 17:40:43Z paul.vandelst@noaa.gov $'
...
!
! PURPOSE:
!       Subroutine to reinitialise the CSEM_LPoly_type structure
!
! CALLING SEQUENCE:
!       CALL CSEM_Clear_LPoly(p)
!
! OUTPUT ARGUMENTS:
!       p:       Reinitialised Lagrangian polynomial structure
!                UNITS:      N/A
!                TYPE:       TYPE(CSEM_LPoly_type)
!                DIMENSION:  Scalar
...
!
!--------------------------------------------------------------------------------
  SUBROUTINE CSEM_Clear_LPoly(p)
    TYPE(CSEM_LPoly_type), INTENT(IN OUT) :: p
    p%Order     = ORDER
    p%nPts      = NPOLY_PTS
    p%lp_left   = ZERO
    p%lp_right  = ZERO
    ...
    p%dx_left   = ZERO
    p%dx_right  = ZERO
  END SUBROUTINE CSEM_Clear_LPoly


  SUBROUTINE CSEM_LPoly_Init(self)
    TYPE(CSEM_LPoly_type), INTENT(OUT) :: self
    self%Order = ORDER
  END SUBROUTINE CSEM_LPoly_Init


  SUBROUTINE CSEM_LPoly_Inspect( self )
    TYPE(CSEM_LPoly_type), INTENT(IN) :: self
  ...
  END SUBROUTINE CSEM_LPoly_Inspect
...
!
! CALLING SEQUENCE:
!       CALL CSEM_Interp_1D(z, ulp, z_int)
!       CALL CSEM_Interp_2D(z, ulp, vlp, z_int)
!       CALL CSEM_Interp_3D(z, ulp, vlp, wlp, z_int)
!       CALL CSEM_Interp_4D(z, ulp, vlp, wlp, xlp, z_int)
!
! INPUT ARGUMENTS:
...
!--------------------------------------------------------------------------------
  ! 1-D routine
  SUBROUTINE CSEM_Interp_1D(z, ulp, &  ! Input
                       z_int   )  ! Output
    ! Arguments
    REAL(fp),         INTENT(IN)     :: z(:)
    TYPE(CSEM_LPoly_type), INTENT(IN)     :: ulp
    ...
    z_int = ( ulp%w_left  * ( ulp%lp_left(1) *z(1) + &
            ...
            ( ulp%w_right * ( ulp%lp_right(1)*z(2) + &
                              ulp%lp_right(2)*z(3) + &
                              ulp%lp_right(3)*z(4) ) )
  END SUBROUTINE CSEM_Interp_1D

  ! 2-D routine
  SUBROUTINE CSEM_Interp_2D(z, ulp, vlp, &  ! Input
                       z_int        )  ! Output
    ! Arguments
    REAL(fp),         INTENT(IN)     :: z(:,:)
    TYPE(CSEM_LPoly_type), INTENT(IN)     :: ulp, vlp
    REAL(fp),         INTENT(IN OUT) :: z_int  ! INTENT(IN OUT) to preclude reinitialisation
    ! Local variables
    INTEGER  :: i
    REAL(fp) :: a(NPTS)
    ...
    DO i = 1, NPTS
      CALL CSEM_Interp_1D(z(:,i),ulp,a(i))
    END DO
    ! Interpolate z in w dimension
    CALL CSEM_Interp_1D(a,vlp,z_int)
  END SUBROUTINE CSEM_Interp_2D

  ! 3-D routine
  SUBROUTINE CSEM_Interp_3D(z, ulp, vlp, wlp, &  ! Input
                       z_int             )  ! Output
    ! Arguments
    REAL(fp),         INTENT(IN)     :: z(:,:,:)
    TYPE(CSEM_LPoly_type), INTENT(IN)     :: ulp, vlp, wlp
    REAL(fp),         INTENT(IN OUT) :: z_int  ! INTENT(IN OUT) to preclude reinitialisation
    ! Local variables
    INTEGER  :: i
    REAL(fp) :: a(NPTS)
    ...
    DO i = 1, NPTS
      CALL CSEM_Interp_2D(z(:,:,i),ulp,vlp,a(i))
    END DO
    ! Interpolate a in w dimension
    CALL CSEM_Interp_1D(a,wlp,z_int)
  END SUBROUTINE CSEM_Interp_3D

  ! 4-D routine
  SUBROUTINE CSEM_Interp_4D(z, ulp, vlp, wlp, xlp, &  ! Input
                       z_int                  )  ! Output
    ! Arguments
    REAL(fp),         INTENT(IN)     :: z(:,:,:,:)
    TYPE(CSEM_LPoly_type), INTENT(IN)     :: ulp, vlp, wlp, xlp
    REAL(fp),         INTENT(IN OUT) :: z_int  ! INTENT(IN OUT) to preclude reinitialisation
    ! Local variables
    INTEGER  :: i
    REAL(fp) :: a(NPTS)
    ...
    DO i = 1, NPTS
      CALL CSEM_Interp_3D(z(:,:,:,i),ulp,vlp,wlp,a(i))
    END DO
    ! Interpolate a in x dimension
    CALL CSEM_Interp_1D(a,xlp,z_int)
  END SUBROUTINE CSEM_Interp_4D
...
!
! CALLING SEQUENCE:
!       CALL CSEM_Interp_1D_TL(z, ulp, z_TL, ulp_TL, z_int_TL)
!       CALL CSEM_Interp_2D_TL(z, ulp, vlp, z_TL, ulp_TL, vlp_TL, z_int_TL)
!       CALL CSEM_Interp_3D_TL(z, ulp, vlp, wlp, z_TL, ulp_TL, vlp_TL, wlp_TL, z_int_TL)
!       CALL CSEM_Interp_4D_TL(z, ulp, vlp, wlp, xlp, z_TL, ulp_TL, vlp_TL, wlp_TL, xlp_TL, z_int_TL)
!
! INPUT ARGUMENTS:
!       z:                       Data to interpolate
!                                UNITS:      Variable
...
!                                from previous calls to the CSEM_LPoly_TL() subroutine.
!                                UNITS:      N/A
!                                TYPE:       CSEM_LPoly_type
!                                DIMENSION:  Scalar
!                                ATTRIBUTES: INTENT(IN)
...
!--------------------------------------------------------------------------------
  ! 1-D routine
  SUBROUTINE CSEM_Interp_1D_TL( z   , ulp   , &  ! FWD Input
    ...
    TYPE(CSEM_LPoly_type), INTENT(IN)     :: ulp_TL
  ...
  END SUBROUTINE CSEM_Interp_1D_TL

  ! 2-D routine
  SUBROUTINE CSEM_Interp_2D_TL( z,    ulp   , vlp   , &  ! FWD Input
                           z_TL, ulp_TL, vlp_TL, &  ! TL  Input
                           z_int_TL              )  ! TL  Output
    REAL(fp),         INTENT(IN)     :: z(:,:)
    TYPE(CSEM_LPoly_type), INTENT(IN)     :: ulp, vlp
    REAL(fp),         INTENT(IN)     :: z_TL(:,:)
    TYPE(CSEM_LPoly_type), INTENT(IN)     :: ulp_TL, vlp_TL
    REAL(fp),         INTENT(IN OUT) :: z_int_TL  ! INTENT(IN OUT) to preclude reinitialisation
    ! Local variables
    INTEGER  :: i
    ...
    DO i = 1, NPTS
      CALL CSEM_Interp_1D(z(:,i),ulp,a(i))
      CALL CSEM_Interp_1D_TL(z(:,i),ulp,z_TL(:,i),ulp_TL,a_TL(i))
    END DO
    ! Interpolate z in w dimension
    CALL CSEM_Interp_1D_TL(a,vlp,a_TL,vlp_TL,z_int_TL)
  END SUBROUTINE CSEM_Interp_2D_TL

  ! 3-D routine
  SUBROUTINE CSEM_Interp_3D_TL( z   , ulp   , vlp   , wlp   , &  ! FWD Input
    ...
    TYPE(CSEM_LPoly_type), INTENT(IN)  :: ulp, vlp, wlp
    REAL(fp),         INTENT(IN)  :: z_TL(:,:,:)
    TYPE(CSEM_LPoly_type), INTENT(IN)  :: ulp_TL, vlp_TL, wlp_TL
    REAL(fp),         INTENT(IN OUT) :: z_int_TL  ! INTENT(IN OUT) to preclude reinitialisation
    ! Local variables
    INTEGER  :: i
    REAL(fp) :: a(NPTS), a_TL(NPTS)
    ...
    DO i = 1, NPTS
      CALL CSEM_Interp_2D(z(:,:,i),ulp,vlp,a(i))
      CALL CSEM_Interp_2D_TL(z(:,:,i),ulp,vlp,z_TL(:,:,i),ulp_TL,vlp_TL,a_TL(i))
    END DO
    ! Interpolate a in w dimension
    CALL CSEM_Interp_1D_TL(a,wlp,a_TL,wlp_TL,z_int_TL)
  END SUBROUTINE CSEM_Interp_3D_TL

  ! 4-D routine
  SUBROUTINE CSEM_Interp_4D_TL( z   , ulp   , vlp   , wlp   , xlp   , &  ! FWD Input
    ...
    TYPE(CSEM_LPoly_type), INTENT(IN)  :: ulp, vlp, wlp, xlp
    REAL(fp),         INTENT(IN)  :: z_TL(:,:,:,:)
    TYPE(CSEM_LPoly_type), INTENT(IN)  :: ulp_TL, vlp_TL, wlp_TL, xlp_TL
    REAL(fp),         INTENT(IN OUT) :: z_int_TL  ! INTENT(IN OUT) to preclude reinitialisation
    ! Local variables
    INTEGER  :: i
    REAL(fp) :: a(NPTS), a_TL(NPTS)
    ...
    DO i = 1, NPTS
      CALL CSEM_Interp_3D(z(:,:,:,i),ulp,vlp,wlp,a(i))
      CALL CSEM_Interp_3D_TL(z(:,:,:,i),ulp,vlp,wlp,z_TL(:,:,:,i),ulp_TL,vlp_TL,wlp_TL,a_TL(i))
    END DO
    ! Interpolate a in x dimension
    CALL CSEM_Interp_1D_TL(a,xlp,a_TL,xlp_TL,z_int_TL)
  END SUBROUTINE CSEM_Interp_4D_TL
...
!
! CALLING SEQUENCE:
!       CALL CSEM_Interp_1D_AD( z   , ulp   , &  ! FWD Input
!                          z_int_AD    , &  ! AD  Input
!                          z_AD, ulp_AD  )  ! AD  Output
!
!       CALL CSEM_Interp_2D_AD( z   , ulp   , vlp   , &  ! FWD Input
!                          z_int_AD            , &  ! AD  Input
!                          z_AD, ulp_AD, vlp_AD  )  ! AD  Output
!
!       CALL CSEM_Interp_3D_AD( z   , ulp   , vlp   , wlp   , &  ! FWD Input
!                          z_int_AD                    , &  ! AD  Input
!                          z_AD, ulp_AD, vlp_AD, wlp_AD  )  ! AD  Output
!
!       CALL CSEM_Interp_4D_AD( z   , ulp   , vlp   , wlp   , xlp   , &  ! FWD Input
!                          z_int_AD                            , &  ! AD  Input
...
!                                into the CSEM_LPoly_AD() subroutine.
!                                UNITS:      N/A
!                                TYPE:       REAL(fp)
!                                DIMENSION:  Same rank as z input
!                                ATTRIBUTES: INTENT(IN OUT)
...
!                                Subsequently passed into the CSEM_LPoly_AD()
!                                subroutine.
!                                UNITS:      N/A
!                                TYPE:       CSEM_LPoly_type
!                                DIMENSION:  Scalar
...
!--------------------------------------------------------------------------------
  ! 1-D routine
  SUBROUTINE CSEM_Interp_1D_AD( z   , ulp   , &  ! FWD Input
                           z_int_AD    , &  ! AD  Input
                           z_AD, ulp_AD  )  ! AD  Output
    ! Arguments
    REAL(fp),         INTENT(IN)     :: z(:)
    ...
    TYPE(CSEM_LPoly_type), INTENT(IN OUT) :: ulp_AD
    ! Local variables
  ...
  END SUBROUTINE CSEM_Interp_1D_AD

  ! 2-D routine
  SUBROUTINE CSEM_Interp_2D_AD( z   , ulp   , vlp   , &  ! FWD Input
                           z_int_AD            , &  ! AD  Input
                           z_AD, ulp_AD, vlp_AD  )  ! AD  Output
    ! Arguments
    REAL(fp),         INTENT(IN)     :: z(:,:)
    ...
    REAL(fp),         INTENT(IN OUT) :: z_int_AD
    REAL(fp),         INTENT(IN OUT) :: z_AD(:,:)
    TYPE(CSEM_LPoly_type), INTENT(IN OUT) :: ulp_AD, vlp_AD
    ! Local variables
    INTEGER  :: i
    ...
    CALL CSEM_Interp_1D_AD(a,vlp,z_int_AD,a_AD,vlp_AD)
    ! Adjoint of z interpolation in u dimension for all v
    DO i = 1, NPTS
      CALL CSEM_Interp_1D_AD(z(:,i),ulp,a_AD(i),z_AD(:,i),ulp_AD)
    END DO
  END SUBROUTINE CSEM_Interp_2D_AD

  ! 3-D routine
  SUBROUTINE CSEM_Interp_3D_AD( z   , ulp   , vlp   , wlp   , &  ! FWD Input
                           z_int_AD                    , &  ! AD  Input
                           z_AD, ulp_AD, vlp_AD, wlp_AD  )  ! AD  Output
    ! Arguments
    REAL(fp),         INTENT(IN)     :: z(:,:,:)
    ...
    REAL(fp),         INTENT(IN OUT) :: z_int_AD
    REAL(fp),         INTENT(IN OUT) :: z_AD(:,:,:)
    TYPE(CSEM_LPoly_type), INTENT(IN OUT) :: ulp_AD, vlp_AD, wlp_AD
    ! Local variables
    INTEGER  :: i
    REAL(fp) :: a(NPTS), a_AD(NPTS)
    ...
    CALL CSEM_Interp_1D_AD(a,wlp,z_int_AD,a_AD,wlp_AD)
    ! Adjoint of z interpolation in u and v dimension for all w
    DO i = 1, NPTS
      CALL CSEM_Interp_2D_AD(z(:,:,i),ulp,vlp,a_AD(i),z_AD(:,:,i),ulp_AD,vlp_AD)
    END DO
  END SUBROUTINE CSEM_Interp_3D_AD

  ! 4-D routine
  SUBROUTINE CSEM_Interp_4D_AD( z   , ulp   , vlp   , wlp   , xlp   , &  ! FWD Input
                           z_int_AD                            , &  ! AD  Input
                           z_AD, ulp_AD, vlp_AD, wlp_AD, xlp_AD  )  ! AD  Output
    ! Arguments
    REAL(fp),         INTENT(IN)     :: z(:,:,:,:)
    ...
    REAL(fp),         INTENT(IN OUT) :: z_int_AD
    REAL(fp),         INTENT(IN OUT) :: z_AD(:,:,:,:)
    TYPE(CSEM_LPoly_type), INTENT(IN OUT) :: ulp_AD, vlp_AD, wlp_AD, xlp_AD
    ! Local variables
    INTEGER  :: i
    REAL(fp) :: a(NPTS), a_AD(NPTS)
    ...
    CALL CSEM_Interp_1D_AD(a,xlp,z_int_AD,a_AD,xlp_AD)
    ! Adjoint of z interpolation in u,v,w dimension for all x
    DO i = 1, NPTS
      CALL CSEM_Interp_3D_AD(z(:,:,:,i),ulp,vlp,wlp,a_AD(i),z_AD(:,:,:,i),ulp_AD,vlp_AD,wlp_AD)
    END DO
  END SUBROUTINE CSEM_Interp_4D_AD
...
!         CALL CSEM_Find_Index( x, dx,        &  ! Input
!                          x_int,        &  ! In/Output
!                          i1, i2,       &  ! Output
!                          out_of_bounds )  ! Output
!
!       For irregularly spaced x-data:
!         CALL CSEM_Find_Index( x,            &  ! Input
!                          x_int,        &  ! In/Output
!                          i1, i2,       &  ! Output
!                          out_of_bounds )  ! Output
!
...
!--------------------------------------------------------------------------------
  ...
  SUBROUTINE CSEM_LPoly(x, x_int, p)
    REAL(fp),         INTENT(IN)     :: x(:)  ! Input
    REAL(fp),         INTENT(IN)     :: x_int ! Input
    TYPE(CSEM_LPoly_type), INTENT(IN OUT) :: p     ! Output. INTENT(IN OUT) to preclude reinitialisation
...
!
! CALLING SEQUENCE:
!       CALL CSEM_LPoly_TL( x   , x_int   , &  ! FWD Input
!                      p             , &  ! FWD Input
...
!--------------------------------------------------------------------------------
  ...
  SUBROUTINE CSEM_LPoly_TL(x   , x_int   , p , &
                      x_TL, x_int_TL, p_TL)
    REAL(fp),         INTENT(IN)     :: x(:)      ! FWD Input
    REAL(fp),         INTENT(IN)     :: x_int     ! FWD Input
    TYPE(CSEM_LPoly_type), INTENT(IN)     :: p         ! FWD Input
    REAL(fp),         INTENT(IN)     :: x_TL(:)   ! TL  Input
    REAL(fp),         INTENT(IN)     :: x_int_TL  ! TL  Input
    TYPE(CSEM_LPoly_type), INTENT(IN OUT) :: p_TL      ! TL  Output. INTENT(IN OUT) to preclude reinitialisation
...
!
! CALLING SEQUENCE:
!       CALL CSEM_LPoly_AD( x   , x_int   , &  ! FWD Input
!                      p             , &  ! FWD Input
!                      p_AD          , &  ! AD  Input
...
!--------------------------------------------------------------------------------
  ...
  SUBROUTINE CSEM_LPoly_AD(x   , x_int   , p , &
                      p_AD, x_AD, x_int_AD)
    REAL(fp),         INTENT(IN)     :: x(:)      ! FWD Input
    REAL(fp),         INTENT(IN)     :: x_int     ! FWD Input
    TYPE(CSEM_LPoly_type), INTENT(IN)     :: p         ! FWD Input
    TYPE(CSEM_LPoly_type), INTENT(IN OUT) :: p_AD      ! AD  Input
