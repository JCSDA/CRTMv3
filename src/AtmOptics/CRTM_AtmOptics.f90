...
CONTAINS
  ...
  SUBROUTINE CRTM_AtmOptics_Combine( &
    ...
    Layer_Loop: DO k = 1, AtmOptics%n_Layers
       ...
       Significant_Scattering: IF( AOvar%bs(k) > BS_THRESHOLD .AND. AOvar%Optical_Depth(k) > 1.0e-12_fp ) THEN
          
          AOvar%w(k) = AtmOptics%Single_Scatter_Albedo(k) / AOvar%Optical_Depth(k)
          ...
          ! Clamp delta so (1 - d*w) stays positive and compute denom
          d_safe = AtmOptics%Delta_Truncation(k)
          IF (AOvar%w(k) > ZERO) THEN
             d_safe = MIN( d_safe, (ONE - 1.0e-12_fp)/AOvar%w(k) )
          END IF
          denom_dw = ONE - ( d_safe * AOvar%w(k) )
          
          ! Redfine the total optical depth and single scattering
          ! albedo for the delta-function adjustment
          AtmOptics%Optical_Depth(k) = denom_dw * AtmOptics%Optical_Depth(k)
          AtmOptics%Single_Scatter_Albedo(k) = ( ONE - d_safe ) * AOvar%w(k) / denom_dw
    ...
    Layer_Loop: DO k = 1, AtmOptics%n_Layers
       ...
       Significant_Scattering: IF( AOvar%bs(k) > BS_THRESHOLD .AND. AOvar%Optical_Depth(k) > 1.0e-12_fp ) THEN
          ...
          AtmOptics_TL%Optical_Depth(k) = &
               ( denom_dw * AtmOptics_TL%Optical_Depth(k) ) - &
               ( d_safe * AOvar%Optical_Depth(k) * w_TL ) - &
