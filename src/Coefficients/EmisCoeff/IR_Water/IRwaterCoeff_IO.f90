...
! IRwaterCoeff_IO
!
! Container module for Binary and netCDF IRwaterCoeff I/O modules.
!
!
! CREATION HISTORY:
!
...
MODULE IRwaterCoeff_IO
    ...
    USE IRwaterCoeff_netCDF_IO , ONLY: IRwaterCoeff_netCDF_InquireFile , &
    ...
    ! ------------
    PRIVATE
    PUBLIC :: IRwaterCoeff_InquireFile_IO
    PUBLIC :: IRwaterCoeff_ReadFile_IO
    PUBLIC :: IRwaterCoeff_WriteFile_IO
    PUBLIC :: IRwaterCoeff_netCDF_to_Binary
    PUBLIC :: IRwaterCoeff_Binary_to_netCDF
...
!
! NAME:
!       IRwaterCoeff_InquireFile_IO
!
! PURPOSE:
!       Function to inquire IRwaterCoeff object files.
!
! CALLING SEQUENCE:
!       Error_Status = IRwaterCoeff_InquireFile_IO( &
!                        Filename, &
!                        netCDF           = netCDF          , &
!                        n_Angles         = n_Angles        , &
...
!------------------------------------------------------------------------------
  ...
  RESULT( err_stat )
    ...
    IF ( Binary ) THEN
      err_stat = IRwaterCoeff_InquireFile(            &
                   Filename                         , &
                   n_Angles        = n_Angles       , &
                   n_Frequencies   = n_Frequencies  , &
                   n_Wind_Speeds   = n_Wind_Speeds  , &
  ...
  END FUNCTION IRwaterCoeff_InquireFile_IO

!------------------------------------------------------------------------------
!:sdoc+:
!
! NAME:
!       IRwaterCoeff_ReadFile_IO
!
! PURPOSE:
!       Function to read IRwaterCoeff object files.
!
! CALLING SEQUENCE:
!       Error_Status = IRwaterCoeff_ReadFile_IO( &
!                        IRwaterCoeff, &
!                        Filename, &
!                        netCDF  = netCDF , &
!                        Quiet   = Quiet  , &
...
!:sdoc-:
!------------------------------------------------------------------------------
  FUNCTION IRwaterCoeff_ReadFile_IO( &
    IRwaterCoeff, &  ! Output
    Filename    , &  ! Input
    netCDF      , &  ! Optional input
  ...
  RESULT( err_stat )
    ...
    IF ( Binary ) THEN
      err_stat = IRwaterCoeff_ReadFile( &
                    ...
                    Quiet       )
    ELSE
      err_stat = IRwaterCoeff_netCDF_ReadFile( &
                    IRwaterCoeff, &
                    Filename    , &
                    Quiet       , &
                    Title       , &
  ...
  END FUNCTION IRwaterCoeff_ReadFile_IO
...
!
! CALLING SEQUENCE:
!       Error_Status = IRwaterCoeff_WriteFile_IO( &
!                        IRwaterCoeff, &
!                        Filename, &
!                        netCDF  = netCDF , &
!                        Quiet   = Quiet  , &
...
!------------------------------------------------------------------------------
  ...
  FUNCTION IRwaterCoeff_WriteFile_IO( &
    IRwaterCoeff, &  ! Input
    Filename    , &  ! Input
    netCDF      , &  ! Optional input
    No_Close    , &  ! Optional input
  ...
  RESULT ( err_stat )
    ...
    ! Call the appropriate function
    IF ( Binary ) THEN
      err_stat = IRwaterCoeff_WriteFile( &
                    IRwaterCoeff, &
                    Filename  , &
                    No_Close  , &
                    Quiet     , &
                    ...
                    Debug     )
    ELSE
      err_stat = IRwaterCoeff_netCDF_WriteFile( &
                    IRwaterCoeff, &
                    Filename  , &
                    Quiet     , &
                    Title     , &
  ...
  END FUNCTION IRwaterCoeff_WriteFile_IO

!------------------------------------------------------------------------------
!:sdoc+:
!
! NAME:
!       IRwaterCoeff_netCDF_to_Binary
!
! PURPOSE:
!       Function to convert a netCDF IRwaterCoeff file to Binary format.
!
! CALLING SEQUENCE:
!       Error_Status = IRwaterCoeff_netCDF_to_Binary( &
!                        NC_Filename  , &
!                        BIN_Filename , &
!                        Quiet = Quiet  )
!
...
!                       netCDF format IRwaterCoeff data file to read.
!                       UNITS:      N/A
!                       TYPE:       CHARACTER(*)
!                       DIMENSION:  Scalar
!                       ATTRIBUTES: INTENT(IN)
...
!                       Binary format IRwaterCoeff data file to write.
!                       UNITS:      N/A
!                       TYPE:       CHARACTER(*)
!                       DIMENSION:  Scalar
!                       ATTRIBUTES: INTENT(IN)
...
!------------------------------------------------------------------------------
  ...
  RESULT( err_stat )
    ...
    INTEGER :: err_stat
    ! Function parameters
    CHARACTER(*), PARAMETER :: ROUTINE_NAME = 'IRwaterCoeff_netCDF_to_Binary'
    ! Function variables
    CHARACTER(256) :: msg
    TYPE(IRwaterCoeff_type) :: cc, cc_copy

    ! Set up
    err_stat = SUCCESS

    ! Read the netCDF file
    err_stat = IRwaterCoeff_ReadFile_IO(cc, NC_Filename, Quiet = Quiet, netCDF = .TRUE. )
    ...
    ! ...Compare the IRwaterCoeff objects
    IF ( .NOT. (cc == cc_copy) ) THEN
      msg = 'IRwaterCoeff object comparison failed.'
      CALL Display_Message( ROUTINE_NAME, msg, err_stat )
      RETURN
    END IF

  END FUNCTION IRwaterCoeff_netCDF_to_Binary

!------------------------------------------------------------------------------
!:sdoc+:
!
! NAME:
!       IRwaterCoeff_Binary_to_netCDF
!
! PURPOSE:
!       Function to convert a binary IRwaterCoeff file to Binary format.
!
! CALLING SEQUENCE:
!       Error_Status = IRwaterCoeff_Binary_to_netCDF( &
!                        BIN_Filename , &
!                        NC_Filename  , &
!                        Quiet = Quiet  )
!
...
!------------------------------------------------------------------------------
  ...
  FUNCTION IRwaterCoeff_Binary_to_netCDF( &
    BIN_Filename, &  ! Input
    NC_Filename , &  ! Input
    Quiet       ) &  ! Optional input
  RESULT( err_stat )
    ...
    INTEGER :: err_stat
    ! Function parameters
    CHARACTER(*), PARAMETER :: ROUTINE_NAME = 'IRwaterCoeff_Binary_to_NetCDF'
    ! Function variables
    CHARACTER(256) :: msg
    TYPE(IRwaterCoeff_type) :: cc, cc_copy
    ...
    ! Write the Binary file
    err_stat = IRwaterCoeff_WriteFile_IO(cc, NC_Filename, Quiet = Quiet, netCDF = .TRUE.)
    IF ( err_stat /= SUCCESS ) THEN
    ...
    ! Check the write was successful
    ! ...Read the Binary file
    err_stat =  IRwaterCoeff_ReadFile_IO(cc_copy, NC_Filename, Quiet = Quiet, netCDF = .TRUE.)
    IF ( err_stat /= SUCCESS ) THEN
  ...
  END FUNCTION IRwaterCoeff_Binary_to_netCDF

END MODULE IRwaterCoeff_IO
