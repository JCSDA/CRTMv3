...
! and their load/destruction routines for the Optical Depth Combining Absorber 
! and Pressure Space (ODCAPS). 
!
! PUBLIC DATA:
...
!                       yong.han@noaa.gov
!
!       Modifed by:     Yong Chen, CIRA/CSU 14-July-2008
!                       yong.chen@noaa.gov
!
MODULE ODCAPS_TauCoeff
  ...
  USE ODCAPS_Define     , ONLY: ODCAPS_TauCoeff_type    => ODCAPS_type, &          
                                ODCAPS_Destroy_TauCoeff => Destroy_ODCAPS        
  USE ODCAPS_Binary_IO  , ONLY: Read_TauCoeff_Binary    => Read_ODCAPS_Binary
  ...
  ! (AtmAbsorption) model
  ! --------------------------------------
  TYPE(ODCAPS_TauCoeff_type), SAVE, ALLOCATABLE, TARGET :: TC(:)
...
!------------------------------------------------------------------------------
  ...
  FUNCTION Destroy_TauCoeff( Process_ID,   &  ! Optional input
    ...
    DO n = 1, SIZE(TC)
      Destroy_Status = ODCAPS_Destroy_TauCoeff( TC(n), &
                                              Message_Log=Message_Log )
