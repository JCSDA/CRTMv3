...
! profiles that cover only a SUBSET of a sensor's channels (channels 2 and 3
! of 'v.abi_gr' here), rather than every channel. CRTM_ChannelInfo_Subset
! restricts CRTM_Forward to process only that subset, so every comparison in
! this program (default vs AOP/COP/TOP) covers just channels 2 and 3.
! Otherwise identical in structure to test_OP.f90: it checks that supplying
! externally-computed aerosol-only (AOP), cloud-only (COP), and combined
! (TOP) optical profiles via CRTM_Options produces RTSolution results
! identical to the default CRTM_Forward calculation, which computes the
! cloud/aerosol scattering internally.
!
! Exercises the OP_Input%Channel_Index / OP_Input_Channel_Position
! channel-mapping fix for CRTMv3 issue #327
! (https://github.com/JCSDA/CRTMv3/issues/327): opt%TOP/COP/AOP%tau etc. only
! have N_CHANNEL_SUBSET columns here, not one per sensor channel, so
! CRTM_Forward_Module must map ChannelIndex (2, then 3) to column position
! (1, then 2) rather than indexing directly with ChannelIndex.
!
! The AOP/COP/TOP netCDF input files are produced by Generate_OP_Subset.f90.



PROGRAM test_OP_Subset
  ...
  ! Parameters
  ! ----------
  CHARACTER(*), PARAMETER :: PROGRAM_NAME      = 'test_OP_Subset'
  ...
  ! The channel subset to process - must match Generate_OP_Subset.f90's
  ! CHANNEL_SUBSET so the AOP/COP/TOP files line up with the channels
  ! CRTM_Forward actually processes below.
  ...
  ! Optical profiles read back from the netCDF files produced by Generate_OP_Subset.f90
  ...
  ! Generate_OP_Subset.f90 - all four calculations must agree on n_Streams
  ! for the comparison in section 6 to be meaningful.
  ...
  CALL Program_Message( PROGRAM_NAME, &
    'Test program for the CRTM Forward function with user defined optical profiles for a channel subset.', &
  ...
  ! 2b. Restrict processing to the channel subset
  ! ----------------------------------------------
  ! CRTM_ChannelInfo_Subset only turns processing ON/OFF per channel (via
  ! Process_Channel) - the underlying SpcCoeff ChannelIndex for channels 2
  ! and 3 stays 2 and 3 (this sensor loads every channel with none skipped,
  ! so Sensor_Channel and ChannelIndex coincide), matching the
  ! Channel_Index values Generate_OP_Subset.f90 wrote into the AOP/COP/TOP
  ! files.
  Error_Status = CRTM_ChannelInfo_Subset( ChannelInfo(1), Channel_Subset=CHANNEL_SUBSET )
  IF ( Error_Status /= SUCCESS ) THEN
    Message = 'Error restricting CRTM to the channel subset'
    CALL Display_Message( PROGRAM_NAME, Message, FAILURE )
    STOP 1
  END IF
  ...
  IF ( n_Channels /= N_CHANNEL_SUBSET ) THEN
    Message = 'Unexpected n_Channels after CRTM_ChannelInfo_Subset'
    CALL Display_Message( PROGRAM_NAME, Message, FAILURE )
    STOP 1
  END IF
  ! ============================================================================
  ...
  ! Generate_OP_Subset.f90 using that same fixed 16-stream setting.
  ...
  ! n_Streams=16 matches Options_Default above and Generate_OP_Subset.f90's
  ! fixed stream count - all runs being compared must agree on this value.
  ...
  !     be compared against the AOP/COP/TOP runs below. Only channels 2 and
  !     3 are actually processed (see CRTM_ChannelInfo_Subset above).
  ! ----------------------------------------------------------------------------
