...
PROGRAM Generate_OP_Subset
  ...
  ! Parameters
  ! ----------
  CHARACTER(*), PARAMETER :: PROGRAM_NAME      = 'Generate_OP_Subset'
  CHARACTER(*), PARAMETER :: COEFFICIENTS_PATH = './testinput/'
  CHARACTER(*), PARAMETER :: AOP_FILE = 'AOP_Subset.nc'
  CHARACTER(*), PARAMETER :: COP_FILE = 'COP_Subset.nc'
  CHARACTER(*), PARAMETER :: TOP_FILE = 'TOP_Subset.nc'

  ! ============================================================================
  ! 0. **** SOME SET UP PARAMETERS FOR THIS TEST ****
  !
  ! Profile dimensions... must match test_OP_Subset.f90
  ...
  ! The channel subset to generate optical profiles for. These are absolute
  ! SpcCoeff ChannelIndex values for 'v.abi_gr' (which, for this sensor, are
  ! identical to its Sensor_Channel numbers since all its channels are
  ! loaded and none are skipped).
  INTEGER, PARAMETER :: N_CHANNEL_SUBSET = 2
  INTEGER, PARAMETER :: CHANNEL_SUBSET(N_CHANNEL_SUBSET) = (/ 2, 3 /)
  ...
  CALL Program_Message( PROGRAM_NAME, &
    'Program to generate user-defined aerosol/cloud/total optical profile (AOP/COP/TOP) files for a channel subset.', &
    'CRTM Version: '//TRIM(Version) )

  ! Sensor_Id
  Sensor_Id = 'v.abi_gr'
  WRITE( *,'(//5x,"Running CRTM for ",a," sensor, channels ",2(i0,1x))' ) TRIM(Sensor_Id), CHANNEL_SUBSET
  ! ============================================================================
  ...
  ! Number of channels this program will generate optical profiles for -
  ! NOT the sensor's full channel count. This program calls
  ! CRTM_Compute_CloudScatter/AerosolScatter directly (not through
  ! CRTM_Forward's ChannelInfo-driven channel loop), so it doesn't need
  ! CRTM_ChannelInfo_Subset - it just loops over CHANNEL_SUBSET explicitly.
  n_Channels = N_CHANNEL_SUBSET
  ! ============================================================================
  ...
  ! Allocate the OP_Input objects that will hold this channel subset's worth
  ! of aerosol-only (AOP), cloud-only (COP), and combined (TOP) optics - only
  ! N_CHANNEL_SUBSET columns, not the sensor's full channel count. The layer
  ! dimension must match the EXTENDED atmosphere (Atm_x), not the
  ...
  SensorIndex = 1
  ! l is the STORAGE position (1..N_CHANNEL_SUBSET) within AOP/COP/TOP's
  ! tau/bs/kb/pcoeff columns; ChannelIndex is the absolute SpcCoeff index
  ! (2, then 3) used for the physics calls below and recorded into
  ! OP%Channel_Index(l) so CRTM_Forward_Module's OP_Input_Channel_Position
  ! can map ChannelIndex back to position l later.
  Channel_Loop: DO l = 1, N_CHANNEL_SUBSET
    ChannelIndex = CHANNEL_SUBSET(l)
    ...
    ! forced in test_OP_Subset.f90 for the default/AOP/COP/TOP calls alike -
    ! the stream count selects AtmOptics%n_Legendre_Terms and hence the
    ! lOffset slice of the Legendre-coefficient lookup table (see
    ! CRTM_Compute_CloudScatter/AerosolScatter), so it must be identical on
    ! the generating side (here) and the consuming side (test_OP_Subset.f90)
    ! or the phase coefficients come from the wrong table slice.
    !
    ...
    IF ( Atm_x(1)%n_Clouds > 0 ) THEN
      ...
      IF ( Error_Status /= SUCCESS ) THEN
        WRITE( Message,'("Error computing CloudScatter for channel ",i0)' ) ChannelIndex
        CALL Display_Message( PROGRAM_NAME, Message, FAILURE )
        STOP 1
      END IF
    ...
    IF ( Atm_x(1)%n_Aerosols > 0 ) THEN
      ...
      IF ( Error_Status /= SUCCESS ) THEN
        WRITE( Message,'("Error computing AerosolScatter for channel ",i0)' ) ChannelIndex
        CALL Display_Message( PROGRAM_NAME, Message, FAILURE )
        STOP 1
      END IF
    ...
    ! Pack this channel's optics into the three OP_Input objects, at
    ! storage position l (NOT ChannelIndex - AOP/COP/TOP only have
    ! N_CHANNEL_SUBSET columns).
    CALL Pack_AtmOptics( AtmOptics_Cloud,   l, ChannelIndex, COP )
    CALL Pack_AtmOptics( AtmOptics_Aerosol, l, ChannelIndex, AOP )
    CALL Pack_AtmOptics( AtmOptics_Total,   l, ChannelIndex, TOP )
...
CONTAINS
  ...
  ! Copy one channel's computed AtmOptics into the corresponding storage
  ! position within an OP_Input object, and record which absolute
  ! ChannelIndex that position corresponds to (via OP%Channel_Index) so
  ! CRTM_Forward_Module's OP_Input_Channel_Position lookup can find it (see
  ! CRTMv3 issue #327). The Legendre index in Phase_Coefficient is 0-based
  ! (0:n_Legendre_Terms) while OP_Input%pcoeff's Legendre dimension is
  ! 1-based, hence the ileg+1 shift - this matches the convention used when
  ! CRTM_Forward_Module reads Options%TOP/COP/AOP back into AtmOptics.
  SUBROUTINE Pack_AtmOptics( AO, Position, ChannelIndex, OP )
    ...
    OP%Channel_Index(Position) = ChannelIndex

    DO ilay = 1, AO%n_Layers
      OP%tau(Position,ilay) = AO%Optical_Depth(ilay)
      OP%bs(Position,ilay)  = AO%Single_Scatter_Albedo(ilay)
      OP%kb(Position,ilay)  = AO%Backscat_Coefficient(ilay)
      DO iphas = 1, AO%n_Phase_Elements
        DO ileg = 0, AO%n_Legendre_Terms
          ileg1 = ileg + 1
          OP%pcoeff(Position,ilay,iphas,ileg1) = AO%Phase_Coefficient(ileg,iphas,ilay)
        END DO
      END DO
    END DO
  END SUBROUTINE Pack_AtmOptics

END PROGRAM Generate_OP_Subset
