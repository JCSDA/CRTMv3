...
| # | Gap | Evidence |
|---|-----|----------|
| 1 | Reported radiance is total intensity, not the channel's polarized measurement. There is no projection of the Stokes vector onto the channel polarization, so a V-polarized channel reports I where it should report I+Q. **The underlying Stokes vector is correct**: `test_VectorRT_ScalarLimit` asserts I = (Iv+Ih)/2 and Q = (Iv-Ih)/2 against two independent scalar runs at 1.1e-16 and 8.2e-17, which is exactly the statement that I+Q reproduces the pure-V scalar radiance. What is missing is only the reporting projection, not the physics. Note also the asymmetry in the adjoint: `%Radiance` is an OUTPUT alias for `Stokes(1)` but not an INPUT one, so seeding `RTSolution_AD%Radiance` on a vector run does nothing (`Assign_Common_Input_AD` reads `%Stokes` in that branch). Seeding `%Stokes` works, and seeding `%Brightness_Temperature` works because the Planck adjoint deposits into `Radiance_AD(1)` before the accumulation. | `src/RTSolution/Common_RTSolution.f90`, `RTSolution%Radiance = RTSolution%Stokes(1)` inside the `n_Stokes > 1` block; adjoint seeding in `Assign_Common_Input_AD` |
