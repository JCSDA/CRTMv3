...
| PARMIO's polarimetric surface uses the same frame convention as FASTEM | `test_VectorRT_SurfaceFrame` now runs both backends: amsua_n19 channel 1 at 23.8 GHz routes to FASTEM, mwr_aws channel 16 at 325 GHz is above the PARMIO threshold and routes to PARMIO. Both give U and V reaching the solver exactly and the exact even/odd mirror signature. Two independent surface models agreeing is a stronger statement about the convention than either alone. It has to be checked at the surface interface: every shipped sensor above 200 GHz sits on a water-vapour line (only mwr_aws at 325.15 and tms_tropics-01 at 204.78 exist), so PARMIO's polarimetric surface never survives to the top of the atmosphere and no radiative-transfer test can see it. Measured: Stokes U at those channels is 1e-16 to 1e-13, against 3.7e-5 on the FASTEM channels |
